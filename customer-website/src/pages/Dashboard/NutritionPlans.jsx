...
function MealListItem({ meal, onSelect, selected, completed, onToggleComplete }) {
  return (
    <motion.div
      layout
      onClick={() => onSelect(meal._id.toString())}
...
export default function NutritionPage() {
  ...
  const completeMeal = async (meal) => {
    ...
    if (completedMeals.includes(mealIdStr)) {
    ...
    try {
      ...
      setCompletedMeals((prev) => {
        ...
        console.log("Instant state update - completedMeals:", updated);
        return updated;
      });

      toast.success("Meal marked as done permanently!");
    } catch (err) {
      console.error("Complete meal error:", err.response?.data || err);
  ...
  useEffect(() => {
    ...
    const loadProgress = async () => {
      try {
        ...
        // 👇👇👇 YE FIX LAGA DO (safe comparison)
        const currentPlanProgress = userProgress.find((p) => {
          const progressPlanId = p.planId?._id?.toString() || p.planId?.toString();
          return progressPlanId === selectedPlanId.toString();
        });

        console.log("Current plan progress object:", currentPlanProgress);

        let completedIds = [];
        if (currentPlanProgress) {
          completedIds = currentPlanProgress.completedMeals
            .filter(cm => cm.dayIndex === selectedDayIndex)
            .map(cm => cm.mealId?._id?.toString() || cm.mealId?.toString());
        }

        console.log("Final completed meal IDs for this day:", completedIds);
        setCompletedMeals(completedIds);

      } catch (err) {
        console.error("Progress reload error:", err.response?.data || err);
        setCompletedMeals([]);
      } finally {
        setProgressLoading(false);
      }
    ...
    loadProgress();
  }, [selectedPlanId, selectedDayIndex]);
  ...
  const completedCount = currentDay.meals?.filter(m =>
    completedMeals.includes(m._id.toString())
  ).length || 0;
              ...
              <div className="bg-white rounded-2xl p-6 shadow border border-gray-100">
                ...
                <div className="text-center -mt-2">
                  <p className="text-3xl font-bold text-gray-900">
                    {progressLoading ? "--" : Math.round(progress * 100)}%
                  </p>
                  <p className="text-sm text-gray-500">
                    {progressLoading ? "Loading..." : "Meals Completed"}
                  </p>
                </div>
                    ...
                    <div key={meal._id} className="flex justify-between items-start">
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{
                            background: completedMeals.includes(meal._id.toString())
                              ? "#16a34a"
                              : THEME,
                          }}
                        ></div>
                      ...
                      <span className="text-sm font-medium text-gray-800">
                        {completedMeals.includes(meal._id.toString()) ? "100%" : "0%"}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white rounded-2xl p-4 shadow border border-gray-100 max-h-[420px] overflow-y-auto">
                <div className="space-y-3">
                  {filteredMeals.length === 0 ? (
                    ...
                    <div className="py-10 text-center text-gray-500">Loading progress...</div>
                  ) : (
                    filteredMeals.map((meal) => (
                      <MealListItem
                        ...
                        completed={completedMeals.includes(meal._id.toString())}
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mt-2">
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-semibold transition ${completedMeals.includes(selectedMeal?._id?.toString())

                          ? "bg-green-600 text-white"
                          : "bg-red-600 text-white"
                          }`}
                        style={{
                          background: completedMeals.includes(selectedMeal?._id?.toString()) ? "#16a34a" : THEME,
                        }}
                      >
                        {completedMeals.includes(selectedMeal?._id?.toString()) ? "Completed" : "Mark as Done"}
                      </button>
