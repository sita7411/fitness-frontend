...
export default function AllTrainers() {
  ...
  const fetchTrainers = async () => {
    try {
      const res = await axios.get(
        `${API_URL}/api/trainers?page=${currentPage}&limit=${ITEMS_PER_PAGE}&search=${searchTerm}`
      );
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (modalType === "add") {
        await axios.post(`${API_URL}/api/trainers`, data, {
        ...
        await axios.put(
          `${API_URL}/api/trainers/${selectedTrainer._id}`,
          data,
          {
            headers: { "Content-Type": "multipart/form-data" },
            withCredentials: true,
  ...
  const handleStatusToggle = async (trainer) => {
    ...
    try {
      // 3. Update server
      await axios.patch(
        `${API_URL}/api/trainers/${trainer._id}/status`,
        { status: newStatus },
        { withCredentials: true }
      );
