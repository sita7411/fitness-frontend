// middleware/auth.js → FINAL RECOMMENDED VERSION (December 22, 2025)
// Supports SIMULTANEOUS User + Admin sessions in same browser
// Dynamic token priority based on route → No conflict!
...
export const protect = async (req, res, next) => {
  try {
    ...
    // Detect route type for dynamic priority
    const isAdminRoute = req.originalUrl.startsWith("/api/admin") ||
                         req.path.startsWith("/admin") ||
                         req.baseUrl?.includes("/admin");

    // DYNAMIC PRIORITY: Route decides which token to prefer
    if (isAdminRoute) {
