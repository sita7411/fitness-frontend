  ...
  try {
    ...
    if (!admin) {
      await User.create({
       ...
       role: "admin",
    phone: "9999999999",                  // Dummy but valid format
    gender: "Male",                        // Schema ke according allowed value
    dob: new Date("1990-01-01"),           // Valid date
    adminPermissions: {
      canManageUsers: true,
    ...
    server.listen(PORT, () => {
      ...
      console.log(
        `Socket.IO is fully active and ready for real-time notifications 🚀`
      );
    });
  } catch (err) {
