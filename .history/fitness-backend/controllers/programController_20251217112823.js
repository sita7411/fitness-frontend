...
export const getUserPrograms = async (req, res) => {
  try {
    ...
    if (user.purchasedPrograms?.length > 0) {
      for (const p of user.purchasedPrograms) {
        if (p.programId) {
          ...
          // FIRST: Try as MongoDB _id (most common in purchased)
          if (mongoose.Types.ObjectId.isValid(p.programId)) {
            prog = await Program.findById(p.programId).lean();
          }

          // SECOND: If not found, try as custom 'id' field
          if (!prog) {
    ...
    // ENROLLED PROGRAMS - same fix
    if (user.enrolledPrograms?.length > 0) {
      for (const e of user.enrolledPrograms) {
        if (e.programId) {
          let prog = null;

          if (mongoose.Types.ObjectId.isValid(e.programId)) {
            prog = await Program.findById(e.programId).lean();
          }

          if (!prog) {
            prog = await Program.findOne({ id: e.programId }).lean();
          }
