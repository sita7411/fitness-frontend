...
// ====== NOTIFICATIONS ON EVENT CREATE ======
    try {
      ...
      // 1. User को notification (जिसके लिए event बना)
      await sendNotification(
        createdEvent.userId,
        "user",
        "New Workout Scheduled 📅",
        `${creatorName} ने आपके लिए "${createdEvent.sourceName || "Workout"}" scheduled किया है। Date: ${new Date(createdEvent.date).toLocaleDateString('en-IN')}`,
        "success",
        "calendar" // ← नया icon
      );

      // 2. सभी Admins को notification
      ...
      for (const admin of admins) {
        await sendNotification(
          ...
          "admin",
          "New Schedule Event",
          `${user?.name || "A member"} के लिए नया event scheduled: "${createdEvent.sourceName || "Manual Workout"}" on ${new Date(createdEvent.date).toLocaleDateString('en-IN')}`,
          "success",
          "calendar"
        );
      }
      ...
      // Fail silently – event तो बन गया है
    }
