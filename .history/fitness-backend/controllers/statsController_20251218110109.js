  ...
  try {
    ...
    if (!stats) {
      stats = await DailyStats.create({
        userId: req.user._id,
        date: today,
        completedMeals: [], // ← important: initialize array
      });
    }
    ...
    // ← YE NAYA LOGIC: Prevent duplicates & add mealId
    ...
    res.json({
      ...
      carbs: stats.carbs,
      fats: stats.fats,
      completedMeals: stats.completedMeals, // optional: for debugging
    });
  } catch (e) {
...
export const getTodaySchedule = async (req, res) => {
  try {
    ...
    const Event = mongoose.model("ScheduleEvent"); // assuming tumhara model name "Event" hai
