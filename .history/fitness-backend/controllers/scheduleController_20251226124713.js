  ...
  try {
    ...
    try {
      ...
      await sendNotification(
        ...
        `${creatorName} ने आपके लिए "${
          createdEvent.sourceName || "Workout"
        }" scheduled किया है। Date: ${new Date(
          createdEvent.date
      ...
      for (const admin of admins) {
        await sendNotification(
          ...
          "admin",
          "New Schedule Event",
          `${user?.name || "A member"} के लिए नया event scheduled: "${
            createdEvent.sourceName || "Manual Workout"
          }" on ${new Date(createdEvent.date).toLocaleDateString("en-IN")}`,
          "success",
          "calendar"
        );
      }
