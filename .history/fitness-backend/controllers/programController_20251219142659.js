...
export const createProgram = async (req, res) => {
  try {
    ...
    try {
      ...
      const activeUsers = await User.find({}); // Ya { "membership.isActive": true } for premium only
      for (const user of activeUsers) {
        await sendNotification(
          user._id,
          "New Workout Program Launched! ",
          `"${newProgram.title}" by ${newProgram.trainerName || "our expert trainer"} is now available!`,
          "success",
          "workout"
        );
      }
...
export const updateProgram = async (req, res) => {
  try {
    ...
    try {
      const changes = [];
      if (programData.title && programData.title !== program.title) changes.push("title updated");
      if (programData.price !== undefined && programData.price !== program.price) changes.push("price changed");
      if (files["thumbnail"]) changes.push("new thumbnail");
      ...
      if (changes.length > 0) {
        ...
        for (const user of activeUsers) {
          await sendNotification(
            user._id,
            "Program Updated! ✏️",
            `"${program.title}" has been updated (${changes.join(", ")}). Check the latest version!`,
            "neutral",
            "workout"
          );
        }
      ...
      for (const admin of admins) {
        await sendAdminNotification(
          admin._id,
          "Program Updated",
          `"${program.title}" has been successfully modified.`,
          "neutral"
        );
      }
    } catch (notifyErr) {
      console.error("Notification error after program update:", notifyErr);
    }
...
console.log("✅ New day completion — saving metadata to DailyStats.heartRate");
    ...
    // ====== METADATA SAVE - ONLY ON NEW COMPLETION ======
    if (isNewCompletion) {
      const programThumbnail = program.thumbnail || "https://res.cloudinary.com/dnmkwvo8m/image/upload/v1764665655/fittrack/programs/d5kngehygkrjzcrh5fhi.jpg";
      const workoutTitle = program.title;
      const dayTitle = program.days.find(d => d.day === Number(day))?.title || `Day ${day}`;
...
console.log("✅ Metadata saved to DailyStats.heartRate");
  ...
  console.log(`   Weight: ${weight || 'Not provided'}`);
      dailyStats.weight = weight ? Number(weight) : (dailyStats.weight || req.user.weight || 0);

      // Update workout minutes
      if (workoutMinutes) {
        dailyStats.workoutMinutes = (dailyStats.workoutMinutes || 0) + Number(workoutMinutes);
      }
    ...
    if (isNewCompletion || alreadyCompletedToday) {
      ...
      const user = await User.findById(req.user._id);
      if (user && currentStreak >= 7 && !user.achievements.includes("seven_day_streak")) {
    ...
    if (isNewCompletion) {
      try {
        await sendNotification(
          ...
          `You successfully finished Day ${day} of "${program.title}"${currentStreak >= 3 ? ` – ${currentStreak}-day streak! 🔥` : "! Keep it up!"}`,
          "success",
          "success"
        );
...
export const completeExercise = async (req, res) => {
  try {
    ...
    if (!day || !exerciseId) {
      return res.status(400).json({ message: "day and exerciseId are required" });
    }
    ...
    if (!currentDayObj) {
      ...
      console.log(`📊 Day ${day} progress: ${completedCount}/${totalExercises} exercises completed`);

      if (completedCount === totalExercises) {
        console.log("🎉 ALL EXERCISES COMPLETED! Day is now fully done → Frontend should show popup");
      } else {
    ...
    if (totalExercisesCompleted === 1 && !user.achievements.includes("first_ex")) {
      user.achievements.push("first_ex");
      unlockedNew = true;
      console.log("🏆 Unlocked: First Exercise");
    }

    if (totalExercisesCompleted >= 10 && !user.achievements.includes("ten_ex")) {
