  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        ...
        if (file.fieldname.startsWith("meal-")) {
          ...
          if (!data.days[dayIndex].meals)
            data.days[dayIndex].meals = [];
    ...
    oldPlan.days.forEach((oldDay, dIdx) => {
      oldDay.meals.forEach((oldMeal, mIdx) => {
        if (
          !data.days?.[dIdx]?.meals?.[mIdx]?.thumbnail
        ) {
    ...
    // Remove invalid _id from days, meals, nutrition
if (data.days && Array.isArray(data.days)) {
  data.days.forEach(day => {
    delete day._id; // remove day _id
    if (day.meals && Array.isArray(day.meals)) {
      day.meals.forEach(meal => {
        delete meal._id; // remove meal _id
        if (meal.nutrition) delete meal.nutrition._id; // remove nutrition _id
      });
    }
  });
}
