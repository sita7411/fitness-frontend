...
export const login = async (req, res) => {
  try {
    ...
    if (user.role === "admin") {
      console.log(
        "🚫 [USER LOGIN] BLOCKED → Admin tried customer login:",
        normalizedEmail
      );
...
export const adminLogin = async (req, res) => {
  try {
    ...
    res.json({
      loggedIn: true, // ← YE JARUR ADD KARO (consistency ke liye)
...
export const saveUserProgress = async (req, res) => {
  try {
    ...
    if (!user.completedExercises.includes(exerciseIdStr)) {
      ...
      console.log(
        `✅ [SAVE PROGRESS] Exercise ${exerciseIdStr} saved for program ${programId}`
      );
      res.json({
