...
export const updatePlan = async (req, res) => {
  try {
    ...
    // Fetch old plan first — हमेशा preserve करने के लिए
    const oldPlan = await NutritionPlan.findById(req.params.id);
    if (!oldPlan) return res.status(404).json({ message: "Plan not found" });

    // ---- HANDLE UPLOADED FILES (Cover + Meal Thumbnails) ----
    let newCoverUploaded = false;
        ...
        if (file.fieldname === "coverImage") {
          ...
          if (parts.length === 3) {
            ...
            if (
              ...
              // नया thumbnail upload हुआ → overwrite करो
              data.days[dayIndex].meals[mealIndex].thumbnail = {
                url: file.path,
                fileName: file.filename,
              };
    ...
    // Preserve old cover if no new one uploaded
    if (!newCoverUploaded) {
      data.coverImage = oldPlan.coverImage;
    }

    // Preserve meal thumbnails if not updated
    // data.days आया है frontend से → उसके meals में thumbnail सिर्फ url या null है
    // इसलिए हमें पुराने thumbnails preserve करने हैं जब तक नया file न आए
    if (data.days && Array.isArray(data.days)) {
      data.days.forEach((day, dayIdx) => {
        if (!day.meals || !Array.isArray(day.meals)) return;

        day.meals.forEach((meal, mealIdx) => {
          const oldMeal = oldPlan.days?.[dayIdx]?.meals?.[mealIdx];
          // अगर frontend ने thumbnail भेजा ही नहीं या सिर्फ url भेजा → पुराना preserve करो
          if (
            meal.thumbnail === null ||
            typeof meal.thumbnail === "string" ||
            !meal.thumbnail?.fileName
          ) {
            if (oldMeal?.thumbnail) {
              meal.thumbnail = oldMeal.thumbnail; // पुराना पूरा object डाल दो
            }
          }
          // अगर नया file upload हुआ → ऊपर already set हो चुका है
        });
      });
    }
    ...
    const updatedPlan = await NutritionPlan.findByIdAndUpdate(
      req.params.id,
      data,
      { new: true, runValidators: true } // validators on रखो अब safe है
    );
