...
export const verifyOtpAndSaveOrder = async (req, res) => {
  ...
  console.log("req.body:", req.body); // ← सबसे important — पूरा body देखो
  try {
    const {
      email: rawEmail,
      otp,
...
// ====================== VERIFY OTP + PROCESS PAYMENT (WITH DEBUG LOGS) ======================
export const verifyOtpAndSaveOrder = async (req, res) => {
  console.log("\n=== verifyOtpAndSaveOrder API HIT ===");
  ...
  try {
    ...
    console.log("\n--- Extracted Values ---");
    console.log("membershipId:", membershipId, "(type:", typeof membershipId, ")");
    console.log("programs.length:", programs.length);
    console.log("classes.length:", classes.length);
    console.log("challenges.length:", challenges.length);
    console.log("nutrition.length:", nutrition.length);
    ...
    if (loggedInUser) console.log("User email:", loggedInUser.email);
    ...
    // ==================== 1. MEMBERSHIP PURCHASE ====================
    if (membershipId) {
      console.log(">>> ENTERED MEMBERSHIP PURCHASE BLOCK <<<");
      ...
      if (loggedInUser) {
        console.log(">>> Assigning membership to user:", loggedInUser.email);
        loggedInUser.membership = membershipInfo;
        loggedInUser.markModified("membership");
        ...
        console.log(`>>> Found ${matchingPrograms.length} programs from membership`);
        ...
        await loggedInUser.save();
        console.log(">>> Membership & included programs saved successfully!");
      }
    } else {
      console.log(">>> NO membershipId — skipping membership block");
    }

    // ==================== 2. NON-MEMBERSHIP ITEMS ASSIGNMENT ====================
    console.log("\n>>> CHECKING NON-MEMBERSHIP ASSIGNMENT CONDITION <<<");
    console.log("!membershipId:", !membershipId);
    console.log("loggedInUser exists:", !!loggedInUser);
    console.log("Condition result (!membershipId && loggedInUser):", !membershipId && loggedInUser);

    if (!membershipId && loggedInUser) {
      console.log(">>> ✅ ENTERED NON-MEMBERSHIP ASSIGNMENT BLOCK <<<");

      // Initialize arrays
      loggedInUser.purchasedPrograms = loggedInUser.purchasedPrograms || [];
      loggedInUser.assignedWorkouts = loggedInUser.assignedWorkouts || [];
      loggedInUser.purchasedClasses = loggedInUser.purchasedClasses || [];
      loggedInUser.assignedClasses = loggedInUser.assignedClasses || [];
      loggedInUser.purchasedChallenges = loggedInUser.purchasedChallenges || [];
      loggedInUser.assignedChallenges = loggedInUser.assignedChallenges || [];
      loggedInUser.purchasedNutritionPlans = loggedInUser.purchasedNutritionPlans || [];
      loggedInUser.assignedNutritionPlans = loggedInUser.assignedNutritionPlans || [];

      console.log(">>> Processing purchased items...");

      if (programs.length > 0) {
        console.log(`>>> Assigning ${programs.length} program(s)`);
        // तुम्हारा programs loop वैसा ही
      }

      if (classes.length > 0) {
        console.log(`>>> Assigning ${classes.length} class(es)`);
        // classes loop
      }

      if (challenges.length > 0) {
        console.log(`>>> Assigning ${challenges.length} challenge(s)`);
        // challenges loop
      }

      if (nutrition.length > 0) {
        console.log(`>>> Assigning ${nutrition.length} nutrition plan(s)`);
        // nutrition loop
      }

      await loggedInUser.save();
      console.log(">>> NON-MEMBERSHIP ITEMS ASSIGNED & USER SAVED SUCCESSFULLY! <<<");
    } else {
      console.log(">>> ❌ SKIPPED NON-MEMBERSHIP ASSIGNMENT BLOCK");
      if (membershipId) console.log("   → Reason: membershipId is present");
      if (!loggedInUser) console.log("   → Reason: User not logged in");
    }
    ...
    const order = await Order.create({
      ...
      programs: orderItems.length > 0 ? orderItems : [{ title: "Purchase", price: total }],
      total,
      status: "Completed",
      paymentMethod: "card",
      isMembershipPurchase: !!membershipId,
    ...
    return res.status(201).json({
      ...
      message: membershipId
        ? "Membership activated successfully!"
        : nutrition.length > 0 && programs.length + classes.length + challenges.length === 0
        ? "Nutrition Plan purchased successfully!"
        : classes.length > 0 && programs.length + challenges.length + nutrition.length === 0
        ? "Class purchased!"
        : programs.length > 0 && classes.length + challenges.length + nutrition.length === 0
        ? "Program purchased successfully!"
        : "Purchase completed successfully! Check your dashboard.",
      orderId: order._id,
    });
