...
// statsController.js → getLatestWorkoutSession (नया सही वाला)
...
export const getLatestWorkoutSession = async (req, res) => {
  try {
    ...
    // Latest day with workoutMinutes > 0
    const recentStats = await DailyStats.find({
      ...
      .limit(1); // सिर्फ latest एक दिन चाहिए
    ...
    // heartRate array में आखिरी entry से title और thumbnail निकालो
    if (latest.heartRate && latest.heartRate.length > 0) {
      const lastEntry = latest.heartRate[latest.heartRate.length - 1];

      if (lastEntry.programTitle) {
        workoutTitle = lastEntry.programTitle;
        if (lastEntry.dayTitle) {
          workoutTitle += ` – ${lastEntry.dayTitle}`;
        }
      }
      if (lastEntry.thumbnail && lastEntry.thumbnail.trim() !== "") {
        thumbnail = lastEntry.thumbnail.trim();
      }
    }
    ...
    const estimatedCalories = Math.round(latest.workoutMinutes * 9); // average 9 kcal/min

    const avgHR = latest.heartRate?.length > 0
      ? Math.round(latest.heartRate.reduce((sum, hr) => sum + hr.value, 0) / latest.heartRate.length)
      : null;
