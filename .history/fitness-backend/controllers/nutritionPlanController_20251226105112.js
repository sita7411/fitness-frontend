...
// POST /api/nutrition/mark-meal-complete
export const markMealComplete = async (req, res) => {
  try {
    ...
    const result = await User.findOneAndUpdate(
      { _id: userId, "nutritionProgress.planId": planId },
      {
        $addToSet: {
    ...
    // Agar plan progress mein nahi mila → pehli baar complete kar raha hai
    if (!result) {
    ...
    // Optional: Daily stats ke liye bhi calories add karo (existing logic)
    // await addDailyMealStats(...)
