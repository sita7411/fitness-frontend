...
export const completeProgramDay = async (req, res) => {
  try {
    ...
    if (!alreadyCompletedToday) {
      ...
      await program.save();  // पहले save करो
    isNewCompletion = true;  // फिर flag set करो
}

      // ====================== FIX: SAVE TITLE & THUMBNAIL FOR LATEST SESSION ======================
      if (isNewCompletion) { 
        ...
        // Push metadata for latest session (dummy HR value, real title + thumbnail)
        dailyStats.heartRate.push({
          value: 0, // dummy value, not used if real HR is provided
          programTitle: workoutTitle,
          dayTitle: dayTitle,
          thumbnail: programThumbnail,
        });

        // Update workout minutes in DailyStats
        if (workoutMinutes) {
