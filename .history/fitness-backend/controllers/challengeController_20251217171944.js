...
const transformFrontendToBackend = (
  data,
  thumbnailUrl = null,
  stepImages = {} // { [fieldname]: uploadedPath }
) => {
  ...
  const daysWithImages = (data.days || []).map((day, dayIdx) => ({
    ...
    steps: (day.steps || day.exercises || []).map((step, stepIdx) => {
      // Generate possible fieldnames that frontend might send
      ...
      // Priority:
      // 1. Newly uploaded image → use multer path
      if (uploadedImagePath) {
        finalImage = uploadedImagePath;
      }
      // 2. Existing remote image (already hosted)
      else if (typeof step.image === "string" && step.image.startsWith('http')) {
        finalImage = step.image;
      }
      // 3. Existing object with url (from previous load)
...
export const createChallenge = async (req, res) => {
  try {
    ...
    // SABHI POSSIBLE STEP IMAGE FIELDNAMES KO CATCH KARO
    const stepImages = {};
    req.files?.forEach((f) => {
