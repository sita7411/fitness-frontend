...
export const completeProgramDay = async (req, res) => {
  try {
    ...
    // Check if this specific day already completed today
    ...
    // Save metadata to DailyStats (only on new completion)
    if (isNewCompletion) {
      ...
      dailyStats.weight = weight ? Number(weight) : dailyStats.weight || 0;

      if (workoutMinutes) {
        dailyStats.workoutMinutes += Number(workoutMinutes);
      }
    ...
    const globalStreak = await calculateGlobalStreak(req.user._id);

    // 7-day streak achievement unlock
    let streakJustUnlocked = false;
    if (globalStreak >= 7) {
      const user = await User.findById(req.user._id);
      if (user && !user.achievements.includes("seven_day_streak")) {
        user.achievements.push("seven_day_streak");
        await user.save();
        streakJustUnlocked = true;
      }
    }

    // ====== NOTIFICATIONS (only on new completion) ======
    if (isNewCompletion) {
      try {
        await sendNotification(
          req.user._id,
          "user",
          "Day Completed! 🔥",
          `Great job finishing Day ${day} of "${program.title}"! Your current streak: ${globalStreak} days`,
          "success",
          "success"
        );
        ...
        if (streakJustUnlocked) {
          for (const admin of admins) {
            await sendNotification(
              ...
              `${req.user.name || "A user"} achieved a 7-day streak!`,
              "success",
              "workout"
            );
          }
    ...
    res.json({
      ...
      day: Number(day),
      streak: globalStreak, // Ab yeh global streak hai
    });
  } catch (error) {
...
export const getUserStreak = async (req, res) => {
  try {
    ...
    const streak = await calculateGlobalStreak(req.user._id);
