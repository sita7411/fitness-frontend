...
export const completeProgramDay = async (req, res) => {
  ...
  const DailyStats = mongoose.model("DailyStats"); // agar top pe import nahi hai
  ...
  // YE HAMESHA PUSH HONA CHAHIYE — HR NA BHI HO TO BHI
  dailyStats.heartRate.push({
    value: heartRate !== undefined && heartRate !== null ? Number(heartRate) : 0,
    programTitle: workoutTitle,
    dayTitle: dayTitle,
    thumbnail: programThumbnail,
    weight: weight !== undefined && weight !== null ? Number(weight) : (req.user.weight || null),
  });

  // Workout minutes add — yeh bhi ensure karo
  if (workoutMinutes) {
  ...
  } else if (!dailyStats.weight || dailyStats.weight === 0) {
    dailyStats.weight = req.user.weight || 0;
  }

  await dailyStats.save();

  console.log("Latest session metadata saved:", {
    title: `${workoutTitle} – ${dayTitle}`,
    thumbnail: programThumbnail,
    weight: dailyStats.weight,
    minutes: dailyStats.workoutMinutes
  });
}
...
export const completeExercise = async (req, res) => {
  try {
    ...
    // Global unique completed exercises count (across all programs)
    ...
    const totalExercisesCompleted = (allCompletedExercises[0]?.total || 0) + 1; // +1 for current exercise
    ...
    res.json({
      ...
      unlockedAchievements: unlockedNew ? user.achievements : undefined, // optional for frontend confetti
    });
  } catch (error) {
...
// controllers/programController.js में add करें
