...
export const completeProgramDay = async (req, res) => {
  try {
    ...
    if (!alreadyCompletedToday) {
      ...
      console.log("✅ New day completion saved in Program.completedBy");
    } else {
      console.log("⚠️ Day already completed today – skipping save");
    }

    // ... (metadata save wala code same rahega)
    ...
    if (isNewCompletion) {
      ...
      try {
        console.log("Sending 'Day Completed' notification to user:", req.user._id);
        await sendNotification(
          req.user._id,
          "user",  // ← YEH IMPORTANT HAI
        ...
        if (currentStreak === 7) {
          console.log("Sending 7-day streak notification to user");
          await sendNotification(
            req.user._id,
            "user",
    ...
    if (isNewCompletion) {
      ...
      for (const admin of admins) {
        try {
          ...
          console.error(`Failed to send to admin ${admin._id}:`, err);
        }
      }
