  ...
  try {
    ...
    if (updates.days && Array.isArray(updates.days)) {
      updates.days = updates.days.map((day, dayIdx) => ({
        ...
        exercises: (day.exercises || []).map((ex, exIdx) => {
          ...
          return {
            ...ex,
            _id: ex._id || undefined, // preserve _id for subdocument updates
            title: ex.title || "Exercise",
            type: ex.type || "time",
            time: ex.time ?? null,
            reps: ex.reps ?? null,
            ...
            notes: ex.notes || "",
            section: ex.section || "Workout",
            thumbnail: newUploadedThumb || thumb || null, // priority: new upload > old real URL > null
          };
        }),
      }));
    }
    ...
    const updatedClass = await ClassModel.findByIdAndUpdate(
      ...
      updates,
      {
        new: true,
        runValidators: true,
        // Important for nested arrays
        overwrite: false, // default, allows partial updates
      }
    );
