...
export const verifyOtpAndSaveOrder = async (req, res) => {
  ...
  try {
    ...
    if (loggedInUser) {
      if (loggedInUser) {
  console.log(">>> ASSIGNING PURCHASED ITEMS TO USER:", loggedInUser.email);
  console.log("Programs:", programs.length, "Classes:", classes.length, "Challenges:", challenges.length, "Nutrition:", nutrition.length);
