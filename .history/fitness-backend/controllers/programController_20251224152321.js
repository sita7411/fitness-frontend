...
export const completeProgramDay = async (req, res) => {
  try {
    ...
    // ====== USER NOTIFICATIONS ======
if (isNewCompletion) {
  console.log("🔥 [NOTIF] Starting USER notification for day completion");

  try {
    console.log("📤 [NOTIF] Sending 'Day Completed' to user:", req.user._id);
    await sendNotification(
      req.user._id,
      "user",
      "Day Completed! Amazing Work! 🔥",
      `You successfully finished Day ${day} of "${program.title}"${
        currentStreak >= 3 ? ` – Keep up your ${currentStreak}-day streak!` : "! Great job!"
      }`,
      "success",
      "success"
    );
    console.log("✅ [NOTIF] User 'Day Completed' notification sent successfully");

    if (currentStreak === 7) {
      console.log("🎉 [NOTIF] Sending 7-day streak to user");
      await sendNotification(
        req.user._id,
        "user",
      ...
      console.log("✅ [NOTIF] 7-day streak notification sent");
    }
  } catch (notifyErr) {
    console.error("❌ [NOTIF] USER NOTIFICATION FAILED:", notifyErr.message);
    console.error("Full error:", notifyErr.stack);
  }
}

// ====== ADMIN NOTIFICATIONS ======
if (isNewCompletion) {
  console.log("👤 [NOTIF] Starting ADMIN notifications");
  const admins = await User.find({ role: "admin" });
  console.log(`[NOTIF] Found ${admins.length} admin(s)`);

  for (const admin of admins) {
    try {
      console.log(`[NOTIF] Sending 'Workout Completed' to admin ${admin._id}`);
      await sendNotification(
        admin._id,
        "admin",
        "Workout Completed",
      ...
      console.log(`✅ [NOTIF] Sent to admin ${admin._id}`);
    } catch (err) {
      console.error(`❌ [NOTIF] Failed to send to admin ${admin._id}:`, err.message);
    }
  }

  if (currentStreak === 7) {
    for (const admin of admins) {
      console.log(`[NOTIF] Sending 'User Achievement' to admin ${admin._id}`);
      await sendNotification(
        admin._id,
        "admin",
        "User Achievement",
