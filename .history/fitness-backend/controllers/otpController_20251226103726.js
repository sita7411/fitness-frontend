...
export const verifyOtpAndSaveOrder = async (req, res) => {
  try {
    ...
    try {
      ...
      let purchaseType = "";
      let purchaseTitle = "";
      let notificationIcon = "activity"; // default fallback

      if (membershipId) {
        purchaseType = "Membership";
        purchaseTitle = membershipInfo?.plan || "Plan";
        notificationIcon = "membership"; // 👑 Crown icon
      } else if (programs.length > 0) {
        purchaseType = "Program";
        purchaseTitle = programs[0].title || "Workout Program";
        notificationIcon = "purchase"; // 🛒 ShoppingCart
      ...
      for (const admin of admins) {
        await sendNotification(
          ...
          notificationMessage,
          "success",
          notificationIcon // ← Dynamic icon: membership ya purchase
        );
      }
