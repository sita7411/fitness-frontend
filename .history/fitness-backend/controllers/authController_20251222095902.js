...
// ---------------- Signup ----------------
// controllers/authController.js → WITH DEBUG LOGS
...
export const signup = async (req, res) => {
  try {
    ...
    console.log("✅ [SIGNUP] Success! User created:", newUser._id, newUser.email);
...
export const login = async (req, res) => {
  try {
    console.log("🔵 [USER LOGIN] Attempt from:", req.body.email);

    const { email, password } = req.body;
    if (!email || !password) {
      console.log("❌ [USER LOGIN] Missing email/password");
    ...
    if (!isMatch) {
      console.log("❌ [USER LOGIN] Wrong password for:", email);
      return res.status(400).json({ message: "Invalid email or password" });
    }

    // CRITICAL: Block admin from customer login
    if (user.role === "admin") {
      console.log("🚫 [USER LOGIN] Admin tried to login via customer endpoint:", email);
    ...
    console.log("✅ [USER LOGIN] Success:", user.email, "(role: user)");
    console.log("🍪 [USER LOGIN] user_token set");
    ...
    console.error("💥 [USER LOGIN] Server Error:", err);
    return res.status(500).json({ message: "Server error" });
  }
};
...
export const adminLogin = async (req, res) => {
  try {
    console.log("🔵 [ADMIN LOGIN] Attempt from:", req.body.email);
    ...
    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) {
      console.log("❌ [ADMIN LOGIN] Wrong password for admin:", email);
      return res.status(400).json({ message: "Invalid credentials" });
    }
    ...
    console.log("✅ [ADMIN LOGIN] Success:", user.email, "(role: admin)");
    console.log("🍪 [ADMIN LOGIN] admin_token set");
...
export const me = async (req, res) => {
  try {
    console.log("🔵 [/me] Called by user ID:", req.user?.id, "Role:", req.user?.role);
    ...
    if (!user) {
      console.log("❌ [/me] User not found in DB:", req.user.id);
      return res.status(401).json({
        loggedIn: false,
        user: null,
    ...
    console.log("✅ [/me] Returning data for:", user.email, "(role:", user.role, ")");
