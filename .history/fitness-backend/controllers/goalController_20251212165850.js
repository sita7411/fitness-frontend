...
export const addGoal = async (req, res) => {
  try {
    const { label, target, color, dataSource = "revenue" } = req.body; // ← YEH ADD KARO
    const period = getCurrentPeriod();
...
export const updateGoal = async (req, res) => {
  try {
    ...
    const { label, target, color, dataSource } = req.body; // ← dataSource add kiya
    const period = getCurrentPeriod();
    ...
    // Sirf agar dataSource bheja gaya ho tab update karo
    if (dataSource) {
...
// ADD THIS FUNCTION IN goalController.js
// controllers/goalController.js → FINAL GOD-LEVEL VERSION
export const getGoalStats = async (req, res) => {
  try {
    ...
    const daysInMonth = new Date(currentYear, currentMonth + 1, 0).getDate(); // last day of month
    ...
    // MONTHLY REVENUE GOAL FETCH FROM DB
    let monthlyRevenueGoal = 2500000; // default fallback
    try {
      ...
      if (goalDoc && goalDoc.goals && goalDoc.goals.length > 0) {
        const revenueGoalObj = goalDoc.goals.find(g => 
    ...
    // DAILY PROGRESS CALCULATION (Overall Monthly Goal ke hisaab se)
    const expectedRevenueTillToday = (monthlyRevenueGoal / daysInMonth) * todayDate;
    const monthRevenue = monthRevenueResult[0]?.total || 0;
    const dailyProgressPercentage = expectedRevenueTillToday > 0 
      ? Math.min(100, Math.round((monthRevenue / expectedRevenueTillToday) * 100))
      : 0;

    const changeVsExpected = expectedRevenueTillToday > 0
      ? Math.round(((monthRevenue - expectedRevenueTillToday) / expectedRevenueTillToday) * 100)
    ...
    res.json({
      ...
      stats: {
        ...
        monthRevenue,
        monthlyRevenueGoal,
        projectedRevenue: Math.round((todayRevenueResult[0]?.total || 0) * daysInMonth),

        // Daily Progress (Main Feature)
