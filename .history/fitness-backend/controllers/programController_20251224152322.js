...
export const completeProgramDay = async (req, res) => {
  try {
    ...
    if (isNewCompletion) {
      ...
      try {
        console.log(
          "📤 [NOTIF] Sending 'Day Completed' to user:",
          req.user._id
        );
        await sendNotification(
          req.user._id,
        ...
        console.log(
          "✅ [NOTIF] User 'Day Completed' notification sent successfully"
        );
        ...
        console.error(
          "❌ [NOTIF] USER NOTIFICATION FAILED:",
          notifyErr.message
        );
    ...
    if (isNewCompletion) {
      ...
      for (const admin of admins) {
        try {
          console.log(
            `[NOTIF] Sending 'Workout Completed' to admin ${admin._id}`
          );
          await sendNotification(
            admin._id,
            "admin",
          ...
          console.error(
            `❌ [NOTIF] Failed to send to admin ${admin._id}:`,
            err.message
          );
        }
      }
      ...
      if (currentStreak === 7) {
        for (const admin of admins) {
          console.log(
            `[NOTIF] Sending 'User Achievement' to admin ${admin._id}`
          );
          await sendNotification(
            admin._id,
            "admin",
