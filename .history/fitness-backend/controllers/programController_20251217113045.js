...
export const getUserPrograms = async (req, res) => {
  try {
    ...
    // In getUserPrograms controller
    if (user.purchasedPrograms?.length > 0) {
      for (const p of user.purchasedPrograms) {
        if (p.programId) {
          let prog = null;

          // FIRST: Try as MongoDB _id (because most purchased use _id)
          if (mongoose.Types.ObjectId.isValid(p.programId)) {
            prog = await Program.findById(p.programId).lean();
          }

          // SECOND: If not found, try custom 'id' field
          if (!prog) {
    ...
    // 2. FREE ENROLLED PROGRAMS - Also fixed
    if (user.enrolledPrograms?.length > 0) {
      for (const e of user.enrolledPrograms) {
        if (e.programId) {
          let prog = await Program.findOne({ id: e.programId }).lean();
          if (!prog) prog = await Program.findById(e.programId).lean();
          if (prog) finalPrograms.push(prog);
        }
      }
    }

    // 3. MEMBERSHIP PROGRAMS (unchanged)
