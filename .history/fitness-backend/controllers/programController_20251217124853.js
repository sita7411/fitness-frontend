...
export const updateProgramStatus = async (req, res) => {
  try {
    ...
    // Add this fallback — this is the missing part!
    if (!program && mongoose.Types.ObjectId.isValid(req.params.id)) {
      program = await Program.findById(req.params.id);
    }
...
export const getUserPrograms = async (req, res) => {
  try {
    ...
    console.log("Membership active:", user.membership?.isActive, "Plan:", user.membership?.plan);
    console.log("Purchased programs count:", user.purchasedPrograms?.length || 0);
    ...
    if (user.membership?.isActive) {
      ...
      if (membershipPrograms.length > 0) {
        membershipPrograms.forEach(mp => {
          console.log(`+ Membership program: ${mp.title} (plans: ${mp.plans?.join(", ")})`);
        });
      }
  ...
  try {
    ...
    const alreadyCompletedToday = program.completedBy.some(
      (entry) =>
        ...
        new Date(entry.completedAt).setHours(0, 0, 0, 0) === today.getTime()
    );
    ...
    if (isNewCompletion) {
      ...
      for (const admin of admins) {
        await notifyAdmin({
          ...
          message: `User ${req.user.name} completed "${program.title}" - ${dayTitle}`,
          type: "success",
          icon: "workout",
        });
      }
...
// controllers/programController.js - NAYA FUNCTION ADD KARO
export const completeExercise = async (req, res) => {
  try {
    ...
    // Program find karo (custom id ya Mongo _id se)
    ...
    // Check karo ki aaj ka ye exercise already complete toh nahi hai
    const alreadyCompleted = program.completedBy.some(entry =>
      entry.user.toString() === req.user._id.toString() &&
      entry.day === Number(day) &&
