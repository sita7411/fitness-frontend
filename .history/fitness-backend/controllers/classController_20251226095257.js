...
export const createClass = async (req, res) => {
  try {
    ...
    try {
      ...
      const allUsers = await User.find({}); // ya active users chahte ho to filter laga sakte ho
      for (const user of allUsers) {
        await sendNotification(
          user._id,
          "user",
          "New Live Class Launched! 🎉",
          `"${savedClass.title}" by ${savedClass.trainerName || "our trainer"} is now available!`,
          "success",
          "class"
        );
      }
...
export const saveClassProgress = async (req, res) => {
  ...
  try {
    ...
    if (!progress) {
      ...
      if (completedExerciseId && !progress.completedExercises.includes(completedExerciseId)) {
        progress.completedExercises.push(completedExerciseId);

        // streak logic
        const yesterday = new Date(today);
        yesterday.setDate(yesterday.getDate() - 1);
        if (progress.lastCompletedDate?.toDateString() === yesterday.toDateString()) {
          progress.streak += 1;
        } else {
          progress.streak = 1;
        }
        ...
        // achievements
        if (progress.completedExercises.length >= 10) progress.achievements.push("ten_ex");
        if (progress.streak >= 7) progress.achievements.push("seven_day_streak");
