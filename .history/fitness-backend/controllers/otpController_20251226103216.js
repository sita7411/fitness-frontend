...
// 3. CREATE ORDER ke baad → YEH CODE ADD KARO
const order = await Order.create({
  // ... sab fields
});
...
try {
  ...
  // User ka naam (logged in ho ya guest)
  ...
  for (const admin of admins) {
    await sendNotification(
      ...
      notificationMessage,
      "success",
      "activity"  // ← Aapka naye green heartbeat icon wala
    );
  }

  console.log(`✅ Admin notification sent for ${purchaseType} purchase by ${buyerName}`);
} catch (notifyErr) {
  console.error("Failed to send admin purchase notification:", notifyErr);
  // Fail silently — purchase to ho gaya hai
}
    if (loggedInUser) {
