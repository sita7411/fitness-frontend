...
export const completeProgramDay = async (req, res) => {
  try {
    // ====== FIX 1: Authentication check (sabse important) ======
    ...
    if (isNewCompletion) {
      ...
      if (
        heartRate ||
        (dailyStats.heartRate && dailyStats.heartRate.length > 0)
      ) {
        ...
        if (validHRs.length > 0) {
          ...
          dailyStats.averageHeartRate = heartRate;
          dailyStats.peakHeartRate = heartRate;
        }
      }
    ...
    यह streak logic completeProgramDay function में isNewCompletion condition के अंदर add करना है।
Exact location जहाँ paste करना है:
JavaScript// ====== METADATA SAVE  ======
    if (isNewCompletion) {
      // ... existing metadata code ...

      await dailyStats.save();  // ← यह line के ठीक बाद
यहाँ paste करो:
JavaScriptawait dailyStats.save();

      // ====== OVERALL WORKOUT STREAK UPDATE (USER LEVEL) ======
      const user = await User.findById(req.user._id);
      if (user) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
    ...
    if (isNewCompletion || alreadyCompletedToday) {
      // Helper function to get YYYY-MM-DD string (timezone safe)
      const getDateString = (date) => {
        const d = new Date(date);
        return d.toLocaleDateString("en-CA"); // "2025-12-25" format
      };
    ...
    // ====== USER NOTIFICATIONS ======
    if (isNewCompletion) {
      console.log("🔥 NOTIFICATION BLOCK ENTERED – isNewCompletion = true");
      ...
      try {
        ...
        if (currentStreak === 7) {
          console.log("🎉 Sending 7-day streak notification...");
          await sendNotification(
            req.user._id,
            "user",
          ...
          console.log("✅ 7-DAY STREAK NOTIFICATION SENT");
        }
      } catch (notifyErr) {
        console.error("❌ USER NOTIFICATION FAILED:", notifyErr);
        console.error("Error details:", notifyErr.message);
        console.error("Stack:", notifyErr.stack);
      }

      // Admin notifications
      console.log("👤 Starting admin notifications...");
      try {
        const admins = await User.find({ role: "admin" });
        console.log(`Found ${admins.length} admins`);

        const userFirstName = req.user.name
          ? req.user.name.trim().split(" ")[0]
          : "A user";
        ...
        for (const admin of admins) {
          console.log(`Sending to admin: ${admin._id}`);
          await sendNotification(
            admin._id,
            "admin",
            "Workout Completed",
          ...
          console.log(`✅ Sent to admin ${admin._id}`);
        }
      ...
      console.log("⚠️ isNewCompletion = false – skipping notifications");
    }
    res.json({
      success: true,
