...
const transformFrontendToBackend = (
  ...
  const daysWithImages = (data.days || []).map((day, dayIdx) => ({
    title: day.title,
    steps: (day.steps || []).map((step, stepIdx) => {
        // Multiple possible keys try karo
        const possibleKeys = [
            `step-${day._id}-${step._id}`,           // existing logic
            `step-day${dayIdx}-step${stepIdx}`,      // common frontend pattern
            `step-${dayIdx}-${stepIdx}`,             // simple index based
            `step-${dayIdx}-${step._id}`,            // mix
            `step-${day._id}-${stepIdx}`
        ];

        let newImage = null;
        for (const key of possibleKeys) {
            if (stepImages[key]) {
                newImage = stepImages[key];
                break;
            }
        }

        const oldImage = step.image && typeof step.image === "object" 
            ? step.image.url 
            : (typeof step.image === "string" ? step.image : null);
        ...
        return {
            ...
            reps: step.reps,
            image: newImage || oldImage || null,  // newImage priority
