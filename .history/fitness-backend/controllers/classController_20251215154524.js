...
const transformFrontendToBackend = (
  ...
  const daysWithThumbnails = (data.days || []).map((day) => ({
    title: day.title,
    exercises: (day.exercises || []).map((ex) => {
const key = `${day.id}_${ex.id}`;      const newThumb = exerciseThumbnails[key];
      const oldThumb =
      ...
      return {
        ...
        sets: ex.sets,
        notes: ex.notes,
        thumbnail: newThumb || oldThumb || null,
  ...
  return {
    ...
    thumbnail: thumbnailUrl || existingThumbnail || null, // ← YEH SABSE ZAROORI
...
export const createClass = async (req, res) => {
  try {
    ...
    console.log(" Main:", mainThumbnail ? "YES" : " NO");
    ...
    req.files?.forEach((f) => {
      if (f.fieldname.startsWith("exercise_")) {
        ...
        console.log(` Exercise: ${f.fieldname} → ${f.path}`);
      }
    });
    console.log(
      " Total exercise thumbnails:",
