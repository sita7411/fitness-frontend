...
export const markMealComplete = async (req, res) => {
  try {
    ...
    if (!mealId || !planId || dayIndex === undefined) {
      return res.status(400).json({ message: "mealId, planId and dayIndex are required" });
    }
    ...
    // Step 1: Try to update existing plan progress
    ...
    // Step 2: If no existing progress found, create new entry
    if (!updatedUser) {
    ...
    // Optional: Ab DailyStats mein bhi add kar do (jo aap pehle kar rahe the)
    // Yeh chhod sakte ho agar sirf nutritionProgress se hi track karna hai

    res.json({ success: true, message: "Meal marked as complete permanently" });
  } catch (err) {
    console.error("Mark meal complete error:", err);
