...
export const me = async (req, res) => {
  try {
    if (!req.user) {
      return res
        .status(401)
        .json({ loggedIn: false, user: null, message: "Not authorized" });
    }

    // Fetch full user with populated assigned items
    ...
    res.json({
      ...
      user: {
        ...
        joined,
        membership: user.membership || { plan: "Free", isActive: false }, // ← YEH ADD KIYA
...
export const getUserPrograms = async (req, res) => {
  try {
    ...
    // Filter only populated programs (successful population)
...
export const adminLogin = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id: user._id, role: user.role }, // ← Ab actual role use karega (admin ho to admin)
      process.env.JWT_SECRET,
      { expiresIn: "7d" }
    );
