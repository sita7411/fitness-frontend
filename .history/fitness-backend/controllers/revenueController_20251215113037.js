...
// Helper: Get total revenue from orders
...
export const getTotalRevenue = async (req, res) => {
  try {
    const { startDate, endDate } = req.query; // Optional: ?startDate=YYYY-MM-DD&endDate=YYYY-MM-DD
...
// 3. Monthly Overview API (Current Month Revenue, Users, Orders, Growth)
...
// 4. Membership Summary API (Total, Active, Expired, Revenue from Memberships)
...
// 5. Plan Distribution API (Popularity of Membership Plans)
...
export const getTransactions = async (req, res) => {
  try {
    ...
    if (type === "program") filter["programs.type"] = "program"; // Assuming programs array has type
...
// 8. Revenue Report API (Comprehensive: Daily, Weekly, Monthly + Breakdown)
export const getRevenueReport = async (req, res) => {
  try {
    ...
    const breakdown = await Order.aggregate([
      ...
      { $unwind: "$programs" }, // Assuming programs array has type
      {
        $group: {
          _id: "$programs.type",
