...
// ------------------ UPDATE CLASS ------------------
// Example: updateClass controller
export const updateClass = async (req, res) => {
  try {
    const { id } = req.params;

    // Jo bhi data body se aaya hai
    let updates = req.body;

    // YE SABSE IMPORTANT LINE HAI
    // Agar new thumbnail file upload hui hai tab hi update karo
    if (req.file) {
      updates.thumbnail = req.file.path; // ya req.file.location if Cloudinary/S3 use kar rahe ho
    } else {
      // Agar file nahi aayi → thumbnail ko updates se nikaal do (purana wala rahega)
      delete updates.thumbnail;
    }

    // Agar koi galti se thumbnail: "" ya null bhej raha hai, usko bhi handle kar lo
    if (
      updates.thumbnail === "" ||
