...
if (meal.prepTime === undefined || meal.prepTime === null) meal.prepTime = 10;
if (meal.cookTime === undefined || meal.cookTime === null) meal.cookTime = 20;
  ...
  try {
    ...
    if (data.days && Array.isArray(data.days)) {
      data.days.forEach((day) => {
        if (day.meals && Array.isArray(day.meals)) {
          day.meals.forEach((meal) => {
            meal.prepTime = meal.prepTime ?? 10;
            meal.cookTime = meal.cookTime ?? 20;
            meal.difficulty = meal.difficulty || "Easy";
            meal.mealType = meal.mealType || "Veg";
            meal.totalTime = meal.totalTime ?? meal.prepTime + meal.cookTime;
          });
        }
      });
