...
export const getLatestWorkoutSession = async (req, res) => {
  try {
    ...
    // Average HR only if entries exist
    ...
    res.json({
      found: true,
      workout: workoutTitle,           // Ab better title dikhega
      calories: Math.round(latest.workoutMinutes * 9),
      duration: latest.workoutMinutes,
      avgHR,                           // Agar HR data hai to dikhega, warna null (sahi hai)
      date: formattedDate,             // Ab sahi format: "Friday, Dec 19"
      thumbnail,                       // Real ya better fallback
      weight: recordedWeight,          // Latest weight dikhega
    });
  } catch (e) {
