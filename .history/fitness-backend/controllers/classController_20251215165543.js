...
export const updateClass = async (req, res) => {
  try {
    ...
    // --- FIX START ---
    // If a new thumbnail file is uploaded, override with Cloudinary URL
    if (req.files && req.files.length > 0) {
      const thumbFile = req.files.find((f) => f.fieldname === "thumbnail");
      if (thumbFile) {
        updates.thumbnail = thumbFile.path;  // Cloudinary string URL
      }
    }

    // If thumbnail is an object (from preview/blob), extract the .url if it's a real URL, or remove it
    if (updates.thumbnail && typeof updates.thumbnail === "object") {
      if (updates.thumbnail.url && updates.thumbnail.url.startsWith("http") && !updates.thumbnail.url.startsWith("blob:")) {
        updates.thumbnail = updates.thumbnail.url;  // keep existing real URL
      } else {
        delete updates.thumbnail;  // ignore blob/preview object
      }
    }

    // Optional: remove empty/null thumbnail
    if (updates.thumbnail === "null" || updates.thumbnail === "" || updates.thumbnail === null) {
