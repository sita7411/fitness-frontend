...
// ====== NOTIFICATIONS ON EVENT CREATE ======
    try {
      ...
      await sendNotification(
        ...
        `${creatorName} has scheduled "${createdEvent.sourceName || "Manual Workout"}" for you on ${eventDate}.`,
        "success",
        "calendar"
      );
      ...
      for (const admin of admins) {
        await sendNotification(
          ...
          `New event scheduled for ${user?.name || "a member"}: "${createdEvent.sourceName || "Manual Workout"}" on ${eventDate}.`,
          "success",
          "calendar"
        );
      }
