...
export const completeProgramDay = async (req, res) => {
  try {
    ...
    // ====== ONLY ON NEW COMPLETION: SAVE METADATA TO DailyStats ======
    if (isNewCompletion) {
      ...
      // Import DailyStats here if not already imported at top
      ...
      // Always push heartRate entry — even if HR is not provided
      dailyStats.heartRate.push({
        value: heartRate !== undefined ? Number(heartRate) : 0,
        programTitle: workoutTitle,
        dayTitle: dayTitle,
        thumbnail: programThumbnail,
        weight: weight !== undefined ? Number(weight) : (req.user.weight || null),
      });
      ...
      // Update daily weight — priority: input > existing > user profile
      ...
      console.log("Metadata saved to DailyStats:", {
        programTitle: workoutTitle,
        dayTitle,
        thumbnail: programThumbnail,
