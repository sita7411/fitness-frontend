...
export const getLatestWorkoutSession = async (req, res) => {
  try {
    ...
    let thumbnail = "https://res.cloudinary.com/dnmkwvo8m/image/upload/v1764665655/fittrack/programs/d5kngehygkrjzcrh5fhi.jpg"; // default fallback
    let recordedWeight = null;

    // Last heartRate entry से metadata + weight निकालो (completeProgramDay से आता है)
    if (latest.heartRate && latest.heartRate.length > 0) {
      ...
      // Weight heartRate entry में saved है तो उसे use करो
      if (lastHR.weight !== undefined && lastHR.weight !== null && lastHR.weight > 0) {
        recordedWeight = Number(lastHR.weight.toFixed(1));
      }
    }

    // Fallback: अगर heartRate में weight न हो तो dailyStats.weight use करो
    ...
    res.json({
      ...
      date: formattedDate,
      thumbnail,
      weight: recordedWeight,  // ← अब weight यहाँ से frontend को मिलेगा
    });
  } catch (e) {
