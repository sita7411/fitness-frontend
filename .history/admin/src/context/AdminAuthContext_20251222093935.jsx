...
const checkAuth = async () => {
  setLoading(true);
  try {
    const res = await api.get("/auth/me");  // ← YEH CHANGE: /admin/me → /me

    console.log("Admin /me response:", res.data);

    if (res.data.loggedIn === true && res.data.user && res.data.user.role === "admin") {
...
const logout = async () => {
  try {
    await api.post("/auth/logout");  // ← YEH CHANGE: /admin/logout → /logout
  } catch (err) {
    console.error(err);
  } finally {
    socket.disconnect();
