...
export default function AdminNotifications({ adminId }) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      console.log("Fetch notifications response:", data); // ✅ LOG
      if (data.success) {
        setNotifications(data.notifications);
      }
    } catch (err) {
      console.error("Fetch notifications error:", err); // ✅ LOG
    }
  };
  ...
  useEffect(() => {
    if (!adminId) {
      console.warn("Admin ID is undefined"); // ✅ LOG
      return;
    }

    console.log("Admin ID:", adminId); // ✅ LOG
    ...
    // SOCKET CONNECT
    socket.on("connect", () => {
      console.log("🟢 SOCKET CONNECTED:", socket.id); // ✅ LOG
    });

    socket.on("disconnect", () => {
      console.log("🔴 SOCKET DISCONNECTED"); // ✅ LOG
    });
    ...
    const handleNewNotification = (notification) => {
      console.log("🔔 NEW ADMIN NOTIFICATION RECEIVED:", notification); // ✅ LOG
  ...
  const markAllRead = async () => {
    try {
      ...
      console.log("Marked all notifications as read"); // ✅ LOG
      setNotifications((prev) =>
        prev.map((n) => ({ ...n, isRead: true }))
      );
    } catch (err) {
      console.error("Mark all read error:", err); // ✅ LOG
    }
  };
  ...
  const deleteNotification = async (id) => {
    try {
      ...
      console.log("Deleted notification ID:", id); // ✅ LOG
      setNotifications((prev) =>
        prev.filter((n) => n._id !== id)
      );
    } catch (err) {
      console.error("Delete notification error:", err); // ✅ LOG
    }
  };
        ...
        <div className="flex gap-6 mb-6 border-b pb-2 text-gray-600 font-medium">
          {["all", "new", "unread"].map((tab) => (
            <button
              ...
              className={`pb-2 ${
                activeTab === tab ? "text-black border-b-2" : "text-gray-500"
              }`}
