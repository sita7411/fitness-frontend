...
export default function AdminDashboard() {
  ...
  // NEW: Top Performer of the Month (Dynamic)
  ...
  const fetchAllData = async () => {
    try {
      ...
      // Weekly Activity (fallback logic same)
    ...
    try {
      ...
      if (res.data && res.data.name && res.data.name !== "No activity yet this month") {
        setTopPerformer({
          ...
          avatar: res.data.avatar || `https://i.pravatar.cc/150?u=${res.data.id || 'top'}`,
          workouts: res.data.workouts || 0,
          calories: res.data.calories || 0,
        });
      } else {
        // Fallback if no one active this month
        setTopPerformer({
  ...
  const handleSaveGoal = async () => {
    if (!newGoal.label || !newGoal.target) {
      alert("Goal name and target are required");
      return;
    }
  ...
  useEffect(() => {
    const initializeDashboard = async () => {
      ...
      try {
        ...
        console.log("Initial live activity load failed");
      }
    };
    ...
    const handleNewNotification = (notification) => {
      if (notification.admin && notification.message) {
        const newActivity = {
        ...
        setLiveActivity(prev => {
          if (prev.some(act => act.message === newActivity.message)) return prev;
  ...
  const handleRefresh = () => {
    setRefreshing(true);
    Promise.all([fetchAllData(), fetchTopPerformer()]).finally(() =>
    ...
    <div className="min-h-screen bg-white rounded-lg p-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          {/* Weekly Activity + Trending */ /* ... unchanged ... */}
          ...
          {/* Daily Progress + Membership Summary */ /* ... unchanged ... */}
            ...
            <div className="bg-white/80 rounded-2xl p-5 shadow flex flex-col items-center">
              ...
              <div className="relative w-32 h-32">
                <svg viewBox="0 0 36 36" className="w-32 h-32 -rotate-90">
                  <path className="text-gray-200" strokeWidth="3.8" fill="none" stroke="#e5e7eb"
                    d="M18 2.0845a15.9155 15.9155 0 1 1 0 31.831 15.9155 15.9155 0 1 1 0-31.831" />
                  <path strokeWidth="3.8" stroke={THEME} fill="none" strokeLinecap="round"
                    d="M18 2.0845a15.9155 15.9155 0 1 1 0 31.831 15.9155 15.9155 0 1 1 0-31.831"
                    strokeDasharray={`${Math.min(dailyProgress.percentage, 100)} 100`} />
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-3xl font-bold text-gray-800">{dailyProgress.percentage}%</div>
                    <div className="text-xs text-gray-500 mt-1">Day {dailyProgress.daysPassed || 1}</div>
                  </div>
                </div>
              </div>
              <div className="mt-3 text-center w-full">
                <div className="text-lg font-bold text-gray-800">₹{dailyProgress.actual.toLocaleString("en-IN")}</div>
                <div className="text-xs text-gray-500">Expected: ₹{dailyProgress.expected.toLocaleString("en-IN")}</div>
              </div>
          ...
          {/* Recent Members Table */ /* ... unchanged ... */}
          <div className="bg-white/80 rounded-2xl p-6 shadow">
        ...
        <div className="space-y-6">
                ...
                goals.map((g, i) => (
                  <div key={g._id || i}>
                    ...
                    <div className="w-full h-2.5 rounded-full bg-gray-200 overflow-hidden">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${g.achieved || 0}%` }}
                        transition={{ duration: 1.2, delay: i * 0.15 }}
                        className="h-2.5 rounded-full" style={{ background: g.color || THEME }} />
                    </div>
                  </div>
                ))
              )}
          ...
          {/* UPDATED: Top Performer Card - Now Dynamic with Avatar */}
          <motion.div whileHover={{ scale: 1.02 }} className="rounded-2xl p-5 relative overflow-hidden shadow text-white"
            ...
            <div className="relative z-10 flex items-start gap-4">
              <div className="w-16 h-16 rounded-full overflow-hidden shadow-lg border-4 border-white/30 flex-shrink-0">
                <img
                  src={topPerformer.avatar || "https://i.pravatar.cc/150"}
              ...
              <div className="flex-1">
                ...
                <h3 className="text-xl font-bold mt-1 truncate pr-2">{topPerformer.name}</h3>
                <div className="grid grid-cols-2 gap-2 mt-3 text-sm">
                  <div>{topPerformer.workouts} Workouts</div>
                  <div>{topPerformer.calories.toLocaleString()} Cal Burned</div>
                </div>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <div className="p-6 border-b">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
                <input type="text" placeholder="Goal Name (e.g. Instagram Followers)" value={newGoal.label}
                  onChange={(e) => setNewGoal({ ...newGoal, label: e.target.value })}
                  className="px-4 py-3 border rounded-xl focus:ring-4 focus:ring-red-100" />
                <input type="number" placeholder="Target (e.g. 50000)" value={newGoal.target}
                  onChange={(e) => setNewGoal({ ...newGoal, target: e.target.value })}
                  className="px-4 py-3 border rounded-xl focus:ring-4 focus:ring-red-100" />
                <select value={newGoal.dataSource || "revenue"}
              ...
              <div className="flex gap-3 mt-4">
                <button onClick={() => { setNewGoal({ label: "", target: "", color: THEME, dataSource: "revenue" }); setEditingGoal(null); }}
                  className="px-4 py-2 border rounded-xl">Clear</button>
            ...
            <div className="p-6">
              ...
              {goals.length === 0 ? (
                ...
                <div className="space-y-3">
                  {goals.map((g, index) => (
                    <div key={g._id || `goal-${index}`} className="flex items-center justify-between bg-gray-50 p-4 rounded-xl">
                      <div>
                        <div className="font-medium">{g.label}</div>
                        <div className="text-sm text-gray-600">Target: {Number(g.target).toLocaleString()}</div>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="px-3 py-1 bg-gray-200 rounded-full text-xs font-medium">{g.achieved || 0}%</span>
                        <button onClick={() => {
                          setNewGoal({ label: g.label, target: g.target, color: g.color || THEME, dataSource: g.dataSource || "revenue" });
                          setEditingGoal(g);
                        }} className="text-blue-600 hover:underline text-sm">Edit</button>
                        <button onClick={() => handleDeleteGoal(g._id)} className="text-red-600 hover:underline text-sm">Delete</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="p-6 pt-0">
              <button onClick={() => setShowManageModal(false)}
                className="w-full py-3 bg-gray-200 hover:bg-gray-300 rounded-xl font-medium">Close</button>
            </div>
          </motion.div>
        </div>
      )}
