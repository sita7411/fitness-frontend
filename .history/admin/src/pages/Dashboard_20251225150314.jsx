...
export default function AdminDashboard() {
  ...
  const handleSaveGoal = async () => {
    ...
    try {
      ...
      // Success → reload goals and reset form
      await loadGoals();
  ...
  const initializeDashboard = async () => {
    ...
    // Load initial live activity with real timestamps
    try {
  ...
  const handleNewNotification = (notification) => {
    if (notification.admin && notification.message) {
      const newActivity = {
        ...
        timestamp: Date.now() // exact time जब notification आई
      };
  ...
  // Optional: Auto-update time every 30 seconds
  const timeUpdateInterval = setInterval(() => {
    setLiveActivity(prev => [...prev]); // trigger re-render
  }, 30000);
