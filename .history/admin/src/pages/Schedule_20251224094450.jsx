...
const getCookie = (name) => {
  ...
  const token = getCookie("admin_token");  const BASE_URL = "http://localhost:5000/api/schedule";

  const fetchUsers = async () => {
    if (!token) {
      console.warn("No token – skipping users fetch");
      return;
    }
    try {
      ...
      if (res.data.success && Array.isArray(res.data.users)) {
        const normalized = res.data.users.map((user) => ({
          ...user,
          _id: user._id || user.id,
          name: user.name || "Unknown User",
        }));
        setAllUsers(normalized);
        console.log("✅ Users loaded:", normalized.length, "users");
      } else {
  ...
  const eventsByDate = useMemo(() => {
    ...
    events.forEach((ev) => {
      let eventUserId = "";
      if (ev.userId) {
        if (ev.userId._id) eventUserId = ev.userId._id.toString();
        else if (typeof ev.userId === "string") eventUserId = ev.userId;
        else eventUserId = ev.userId.toString();
      }
