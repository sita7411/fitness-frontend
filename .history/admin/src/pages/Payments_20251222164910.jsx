...
const API_BASE = "/api/revenue"; // Adjust if your base URL is different
...
export default function PaymentsPage() {
    ...
    const fetchTransactions = async () => {
        setLoading(true);
        try {
            const res = await api.get("/revenue/transactions?limit=50"); // note: api base is already http://localhost:5000/api/admin
            ...
            setInvoices(data.slice(0, 10)); // Reuse recent transactions as invoices
        } catch (err) {
    ...
    const fetchMemberships = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${API_BASE}/membership/users?limit=50`);
    ...
    const fetchRevenuePerformance = async () => {
        setLoading(true);
        try {
            // We'll use last 12 months approximation using monthly performance
