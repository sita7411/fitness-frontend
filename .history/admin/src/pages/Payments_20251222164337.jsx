// src/admin/PaymentsPage.jsx (या admin folder में रखो)
...
export default function PaymentsPage() {
  const { admin, isLoggedIn, loading: authLoading, api } = useAdminAuth();
  ...
  // If not logged in (after check), show nothing or redirect already handled
  if (!isLoggedIn) return null;

  // Fetch functions using admin api instance
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      const res = await api.get(`${API_BASE}/transactions?limit=50`);
  ...
  const fetchMemberships = async () => {
    setLoading(true);
    try {
      const res = await api.get(`${API_BASE}/membership/users?limit=50`);
  ...
  const fetchRevenuePerformance = async () => {
    setLoading(true);
    try {
      const res = await api.get(`${API_BASE}/performance?period=monthly`);
      ...
      try {
        const weeklyRes = await api.get(`${API_BASE}/last-7-days`);
  ...
  const badgeClasses = (status) => {
    switch (status) {
      case "Success": case "Paid": case "Active":
  ...
  const handleDownloadInvoice = async (invoice) => {
    ...
    autoTable(doc, {
      ...
      theme: "grid",
      headStyles: { fillColor: LIGHT, textColor: DARK, fontStyle: "bold", lineColor: BORDER },
      bodyStyles: { fontSize: 10, textColor: DARK, cellPadding: 10, lineColor: BORDER },
      columnStyles: {
    ...
    doc.setFontSize(9);
    doc.setTextColor(GRAY);
    doc.text("This is a computer generated invoice and does not require signature.", 40, endY);
                  ...
                  <td key={j} className={`p-3 border-b text-${col.align || "left"}`}>
                    {col.key === "status" ? (
                      ...
                      <div className="flex gap-3 justify-center">
                        ...
                        {type === "invoices" && (
                          <Download className="text-green-600 hover:text-green-800 cursor-pointer" onClick={() => handleDownloadInvoice(item)} />
                        )}
                        {type !== "invoices" && (
