...
const EventCard = ({ ev, onClick }) => {
  const userName = ev.displayUserName || "Unknown User";
...
export default function AdminSchedule() {
  // 🟢 1. Component mount hua ki nahi?
  console.log("🟢 AdminSchedule component MOUNTED / RENDERED");

  const { admin } = useAdminAuth();
  const token = admin?.token;

  // 🟢 2. Admin login hai ya nahi?
  console.log("🔐 Admin from context:", admin ? "Logged in" : "NOT logged in");
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        console.log("✅ Users fetched:", normalized.length, "users");
      }
    } catch (err) {
 ...
 useEffect(() => {
  ...
  const fetchEvents = async () => {
    // token की check हटाओ — cookie से automatically जाएगा
    // if (!token) { ... } → यह हटा दो

    console.log("📅 Fetching events for month:", format(currentMonth, "MMMM yyyy"));
    console.log("👤 Selected user filter:", selectedUser === null ? "All Users" : selectedUser);
    ...
    try {
      ...
      const res = await axios.get(`${BASE_URL}/admin`, {
        params,
        withCredentials: true, // ← यह काफी है! Cookie भेजेगा
        // headers हटाओ completely
      });

      if (res.data.success) {
        setEvents(res.data.events || []);
        console.log("✅ Events fetched:", res.data.events?.length || 0, "events");
      }
    } catch (err) {
  ...
  fetchEvents();
}, [currentMonth, selectedUser]); // ← token dependency भी हटा दो

  // 🟢 3. Final data state before render
  console.log("📊 Current state → Events:", events.length, "| Users:", allUsers.length, "| Selected User:", selectedUser);
  ...
  const openModal = (date, event = null) => {
    ...
    setNewEvent(
      event
        ? { ...event, userId: event.userId?._id || event.userId || "" }
        : createEmptyEvent()
    );
    setModalOpen(true);
  };
  ...
  const eventsByDate = useMemo(() => {
    ...
    events.forEach((ev) => {
      ...
      if (ev.userId) {
        if (typeof ev.userId === "object" && ev.userId?._id) {
          userName = ev.userId.name || "Unknown User";
      ...
      const shouldInclude =
        selectedUser === null || eventUserId === selectedUser;
    ...
    console.log("🗓️ eventsByDate calculated → keys:", Object.keys(map).sort());

    return map;
  }, [events, selectedUser]); // allUsers removed from dependency
              ...
              {week.map((day) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`rounded-2xl p-3 min-h-[120px] cursor-pointer shadow-sm hover:shadow-md transition border ${
                    ...
                    <div className="flex justify-between mb-2">
                      <div
                        className={`font-semibold text-lg ${
                          isToday(day) ? "text-[#e3002a]" : "text-gray-800"
                        }`}
                      >
                        {format(day, "d")}
                      </div>
