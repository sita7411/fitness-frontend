...
import { useAdminAuth } from "../context/AdminAuthContext"; // Assume path correct
...
export default function AllMembers() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this member?")) {
      try {
        await axios.delete(`/api/auth/${id}`, { withCredentials: true });
  ...
  return (
    <div className="min-h-screen bg-white rounded-lg p-6">
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center">
        ...
        <div className="flex gap-3 flex-wrap">
          <div className="relative w-40">
            ...
            {statusDropdownOpen && (
              <div id="statusDropdown" className="absolute mt-2 w-full bg-white rounded-xl shadow-lg border border-gray-200 z-20 overflow-hidden">
                {["All", "Active", "Expired", "Pending"].map((status) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 transition ${
                      filterStatus === status ? "bg-gray-100 font-semibold" : "text-gray-700"
                    }`}
                  >
                    {status}
                  </button>
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-[#e3002a] text-white flex items-center justify-center font-bold text-lg shadow">
                          {member.avatar}
                        </div>
                        <div>
      ...
      {/* MODAL - Simple version (Add/Edit) */}
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>
          <div className="relative bg-white rounded-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <button 
              onClick={closeModal} 
