      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-4 mb-10">
              {/* Prep Time */}
              <div>
                <label className="text-gray-500 text-xs font-medium">Prep Time (min)</label>
                <input
                  type="number"
                  min="0"
                  value={meal.prepTime || 10}
                  onChange={(e) => onUpdateMeal(dayIndex, meal.id, { prepTime: Number(e.target.value) })}
                  className="w-full p-2 border rounded-lg mt-1 shadow-sm focus:ring-1 focus:ring-red-600 focus:border-red-600 transition"
                  style={{ borderColor: LIGHT_BORDER }}
                />
              </div>
              ...
              {/* Cook Time */}
              <div>
                <label className="text-gray-500 text-xs font-medium">Cook Time (min)</label>
                <input
                  type="number"
                  min="0"
                  value={meal.cookTime || 20}
                  onChange={(e) => onUpdateMeal(dayIndex, meal.id, { cookTime: Number(e.target.value) })}
                  className="w-full p-2 border rounded-lg mt-1 shadow-sm focus:ring-1 focus:ring-red-600 focus:border-red-600 transition"
                  style={{ borderColor: LIGHT_BORDER }}
                />
              </div>
              ...
              {/* Difficulty */}
              <div>
                <label className="text-gray-500 text-xs font-medium">Difficulty</label>
                <Dropdown
                  options={["Easy", "Medium", "Hard"]}
                  value={meal.difficulty || "Easy"}
                  onChange={(v) => onUpdateMeal(dayIndex, meal.id, { difficulty: v })}
                />
              </div>
              ...
              <div>
                ...
                <Dropdown
                  ...
                  onChange={(v) => onUpdateMeal(dayIndex, meal.id, { mealType: v })}
                />
              </div>
            </div>
    ...
    const cleanPlan = {
      ...
      days: plan.days.map(day => ({
        ...
        meals: day.meals.map(meal => ({
          ...
          thumbnailIndex: meal.thumbnail?.file ? mealIdx : null // temporary mapping
        }))
      }))
    };
