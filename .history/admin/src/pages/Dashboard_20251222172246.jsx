...
import { useAdminAuth } from "../context/AdminAuthContext"; // ← ये add करो top में
const THEME = "#e3002a";
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [
        ...
        axios.get('/api/revenue/today', { withCredentials: true }).catch(() => ({ data: { revenue: 124000 } })),
        axios.get('/api/revenue/performance', { withCredentials: true }).catch(() => ({ data: { data: [] } })),
        axios.get('/api/revenue/last-7-days', { withCredentials: true }).catch(() => ({ data: { success: false } })),
