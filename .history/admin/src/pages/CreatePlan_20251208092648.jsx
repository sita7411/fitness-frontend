...
export default function CreateNutritionPlan() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ✅ CLEAN JSON - Backend expects this format
      const cleanPlan = {
        ...
        days: plan.days.map(day => ({
          ...
          meals: day.meals.map(meal => ({
            ...
            ingredients: Array.isArray(meal.ingredients) ? meal.ingredients.filter(Boolean) : [],
            instructions: Array.isArray(meal.instructions) ? meal.instructions.filter(Boolean) : [],
            tools: Array.isArray(meal.tools) ? meal.tools.filter(Boolean) : [],
            notes: Array.isArray(meal.notes) ? meal.notes.filter(Boolean) : [],
            nutrition: {
            ...
            thumbnail: meal.thumbnail?.url || null // Just URL, no file
          }))
        }))
      };
