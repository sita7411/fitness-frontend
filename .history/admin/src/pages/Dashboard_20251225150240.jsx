...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const loadAll = async () => {
      ...
      // Fetch initial live activity from recent notifications
      try {
        ...
        if (res.data.success && res.data.notifications?.length > 0) {
          const messages = res.data.notifications
            .filter(n => n.admin && n.message)
            .map(n => n.message)
            .reverse()
            .slice(0, 6);
          if (messages.length > 0) {
            setLiveActivity(messages);
          }
        }
      } catch (err) {
        console.log("Could not load initial live activity");
      }
    };
    loadAll();
    ...
    const handleNewNotification = (notification) => {
      if (notification.admin && notification.message) {
        ...
        setLiveActivity(prev => {
          ...
          return [message, ...prev].slice(0, 6); // Max 6 items
        });
      }
    };
