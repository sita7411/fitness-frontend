...
export default function AdminSchedule() {
  ...
  const eventsByDate = useMemo(() => {
    ...
    events.forEach((ev) => {
      ...
      // userId null hai → eventUserId empty rahega, lekin userName "Unknown User" ya fallback

      ev.displayUserName = userName;

      // FIX: Jab selectedUser null hai (All Users), toh SAB events dikhao
      // Chahe userId null ho ya na ho
      const shouldInclude =
        selectedUser === null || // All Users selected
        (eventUserId && eventUserId === selectedUser); // Specific user selected
    ...
    return map;
  }, [events, selectedUser, allUsers]);
  console.log("Current Month:", format(currentMonth, "MMMM yyyy"));
  console.log("Total events loaded:", events.length);
  console.log("Events dates:", [...new Set(events.map(ev => ev.date))].sort());
  console.log("eventsByDate keys:", Object.keys(eventsByDate).sort());
  console.log("Sample dayEvents for today:", eventsByDate[format(new Date(), "yyyy-MM-dd")]);
  return (
