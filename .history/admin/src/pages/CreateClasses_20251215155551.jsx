...
export default function CreateClasses() {
    ...
    const handleSave = async () => {
        ...
        // Exercise Thumbnails — Ab index use karo (dayIndex_exIndex)
        classes.classList.forEach((day, dayIndex) => {
            day.exercises.forEach((ex, exIndex) => {
                if (ex.thumbnail?.file) {
                    ...
                    console.log(`Uploading exercise thumbnail: ${fieldName}`);
                }
            });
        });

        // Clean payload — IDs bilkul nahi bhejna
        const payload = {
            ...
            days: classes.classList.map((day) => ({
                ...
                exercises: day.exercises.map((ex) => ({
                    ...
                    // thumbnail old wala (if any) — new backend se aayega
                    thumbnail: typeof ex.thumbnail === "object" && ex.thumbnail?.url ? ex.thumbnail : null,
                })),
            })),
        };
