...
const Navbar = () => {
  ...
  // Fetch logo from settings API only once when component mounts
  useEffect(() => {
    const fetchLogo = async () => {
      try {
        const res = await fetch("/api/settings"); // ← apne route ke hisab se adjust karo (jaise /api/settings)
        if (!res.ok) throw new Error("Failed to fetch settings");

        const data = await res.json();
        if (data.logo) {
          setLogoUrl(data.logo); // Cloudinary URL jo backend se aayega
        }
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex-shrink-0">
            <Link to="/">
              {loadingLogo ? (
                // Simple skeleton loader jab tak logo load ho raha ho
                <div className="h-14 w-40 bg-gray-200 animate-pulse rounded"></div>
              ) : (
                <img
                  className="h-14 mt-3 -ml-7 w-auto" // h-45 ko h-14 kiya (Tailwind mein h-45 nahi hota)
                  src={logoUrl}
                  alt="FitHealth"
                />
              )}
          ...
          {/* Baaki sab code bilkul same rahega... */}
          {/* DESKTOP LINKS, CART, ACCOUNT etc. */}
          {/* (Niche wala code same copy-paste kar do jo pehle tha) */}
