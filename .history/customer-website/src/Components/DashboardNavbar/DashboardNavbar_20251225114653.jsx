...
export default function Navbar() {
  ...
  // Main logic: Fetch + Socket
  useEffect(() => {
    // Agar loading ya logged out hai
    if (loading || !isLoggedIn || !authUser?._id) {
      setNotifications([]);
    ...
    const fetchNotifications = async () => {
      try {
        ...
        console.log("Fetch response:", response.data); // ← Debug ke liye
    ...
    socket.emit("register", { role: "user", id: authUser._id });
    console.log("Registered user on socket:", authUser._id);
    ...
    return () => {
      isMounted = false;
      socket.off("notification:new", handleNewNotification);
      // Optional: disconnect only if no other component uses it
    };
  }, [authUser?._id, isLoggedIn, loading]);
  ...
  const deleteNotification = async (id) => {
    try {
      ...
      console.error("Delete failed in navbar:", err.response?.data || err.message);
    }
  };
              ...
              <div className="max-h-96 overflow-y-auto">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((notif) => {
                    const Icon = iconMap[notif.icon || notif.type] || Bell;
