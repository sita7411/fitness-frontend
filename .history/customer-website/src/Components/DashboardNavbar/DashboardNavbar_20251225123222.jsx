...
export default function Navbar() {
  ...
  // MAIN FIX: यहाँ सभी authUser._id → authUser.id कर दिया
  useEffect(() => {
    if (loading || !isLoggedIn || !authUser?.id) return;  // ← यहाँ .id
    ...
    const fetchNotifications = async () => {
      try {
        ...
        if (response.data.success && isMounted) {
          ...
          console.log("Navbar: Fetched notifications:", response.data.notifications?.length);
        }
      } catch (err) {
    ...
    const registerUser = () => {
      socket.emit("register", { role: "user", id: authUser.id }); // ← यहाँ .id
      console.log("Navbar: Registered user on socket:", authUser.id);
    };
  ...
  }, [authUser?.id, isLoggedIn, loading]); // ← dependency में भी .id
