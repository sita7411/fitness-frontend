...
export default function AuthPage() {
  ...
  const { login, api } = useUserAuth(); // ← api add kiya for signup
    ...
    try {
      if (isLogin) {
        ...
        if (result?.success) {
          ...
          navigate("/"); // ← Instant redirect – no delay
        } else {
          toast.error("Login failed – please try again");
        }
      } else {
        // SIGNUP – using FormData + context api
        ...
        // Future mein avatar add karna ho to:
      ...
      const msg =
        ...
        (isLogin ? "Login failed" : "Signup failed – try again");
      toast.error(msg);
