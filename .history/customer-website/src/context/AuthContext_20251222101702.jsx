  ...
  return (
    <UserAuthContext.Provider
      value={{
        ...
        checkAuth,      // Manual refresh के लिए (optional)
      }}
    >
      {children}
    </UserAuthContext.Provider>
  ...
  const checkAuth = async () => {
    ...
    try {
      ...
      if (
        ...
        setUser(res.data.user);
        setIsLoggedIn(true);
        console.log("✅ [USER CONTEXT] Authenticated as normal user:", res.data.user.email);
      } else {
        setUser(null);
        setIsLoggedIn(false);
        console.log("❌ [USER CONTEXT] Rejected – Not a normal user");
      }
    } catch (err) {
      console.error("❌ [USER CONTEXT] Auth check failed:", err.response?.status || err.message);
      setUser(null);
      setIsLoggedIn(false);
    } finally {
      setLoading(false);
