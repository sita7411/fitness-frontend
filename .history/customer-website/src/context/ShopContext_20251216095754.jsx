...
export const ShopProvider = ({ children }) => {
    ...
    image: item.coverImage?.url || 
           ...
           item.image || 
           item.images?.[0]?.url || 
           "/placeholder-program.jpg",  // better placeholder
    price: Number(item.price) || 0,
    
    // Fixed fallbacks
    duration: item.duration || "4-8 Weeks",
    difficulty: item.difficulty || item.level || "All Levels",  // support both
    trainerName: item.trainerName || "Self-Guided",  // correct for programs
    quantity: 1,
};
