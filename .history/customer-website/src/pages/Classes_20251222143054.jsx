...
const Classes = () => {
  ...
  const fetchClasses = async () => {
    setLoading(true);
    console.log("Fetching from /api/classes/active");

    try {
      const res = await axios.get("http://localhost:5000/api/classes/active");

      console.log("Raw data from backend:", res.data); // ← यह जरूर देखो console में

      const filteredClasses = res.data.filter(cls => cls.status === "Active");
      ...
      console.log("Final classes to display:", transformedClasses);

      setClasses(transformedClasses);

      if (transformedClasses.length === 0) {
        toast.info("No active classes found yet. Check back soon!");
      }
    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-[90vh] flex items-center justify-center text-white">
        ...
        <div className="relative z-10 text-center px-6 mt-16 max-w-3xl">
          ...
          >
            OUR
            <span className="text-[#E3002A]"> CLASSES</span>
          </motion.h1>
          <motion.p
            initial={{ y: 30, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
      ...
      {/* Classes Grid */}
      <div id="classes" className="grid gap-10 mt-20 mb-10 mx-10 md:grid-cols-2 lg:grid-cols-3">
        {loading ? (
          <div className="col-span-full text-center py-20">
            <div className="inline-block animate-spin rounded-full h-12 w-12 border-t-4 border-[#E3002A]"></div>
            <p className="mt-4 text-xl text-gray-600">Loading classes...</p>
          </div>
        ) : currentClasses.length === 0 ? (
          <div className="col-span-full text-center py-20">
            <p className="text-2xl text-gray-500">No classes available right now</p>
            <p className="text-gray-400 mt-4">Check back soon!</p>
          </div>
        ) : (
          currentClasses.map((cls) => (
            <motion.div
key={cls._id.toString()} // ← यही main fix!              whileHover={{ y: -6, scale: 1.02 }}
            ...
            >
              {/* तुम्हारा existing card code same रहेगा */}
