...
const Classes = () => {
  ...
  const fetchClasses = async () => {
    try {
const res = await axios.get("http://localhost:5000/api/classes");       const transformedClasses = res.data.map(cls => {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative h-[80vh] flex items-center justify-center text-white">
        ...
        <div className="relative z-10 text-center mt-25 mb-10 px-6 max-w-3xl">
          <motion.h1
            initial={{ y: 30, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.8 }}
            className="text-5xl md:text-6xl font-extrabold uppercase leading-tight tracking-wide"
          >
            Our <span className="text-[#E3002A]">Classes</span>
          </motion.h1>
          <motion.p
            initial={{ y: 30, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ duration: 0.9, delay: 0.3 }}
            className="text-gray-200 mt-5 text-lg md:text-xl max-w-2xl mx-auto"
          >
