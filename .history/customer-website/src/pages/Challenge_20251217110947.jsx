...
const ChallengesPage = () => {
      ...
      try {
        ...
        setChallengesData(
          data.map((ch) => ({
            ...
            images: [ch.thumbnail || "/challenges-placeholder.jpg"],
            difficulty: ch.difficulty || "Beginner",
            free: ch.price === 0,
            price: ch.price || 0,
            trainer: "Self-Guided",
            duration: `${ch.totalDays || ch.days?.length || 1} day${(ch.totalDays || ch.days?.length || 1) > 1 ? "s" : ""}`,
  ...
  const handleAddToCart = (challenge) => {
    if (challenge.free) {
      toast.success("Free challenge activated! You can start it anytime.");
      // In future: auto-enroll free challenges here if needed
      return;
    }
    ...
    const cartItem = {
      ...
      image: challenge.images[0],
      price: challenge.price,
      duration: challenge.duration,
      difficulty: challenge.difficulty,
      category: "Challenge",
      trainerName: challenge.trainer,
      quantity: 1,
    };

    addToCart(cartItem);
    toast.success(`${challenge.name} added to cart!`);
  };
  ...
  if (loading)
    return <div className="text-center mt-20 text-xl text-gray-600">Loading challenges...</div>;
    ...
    <div className="min-h-screen">
      ...
      <section className="relative h-[90vh] flex items-center justify-center text-white">
        ...
        <div className="relative z-10 text-center mt-16 max-w-3xl px-6">
          ...
          >
            Join challenges and improve your fitness. Free challenges are ready to start anytime!
          </motion.p>
          <motion.div
      ...
      {/* Challenges Grid */}
      <div id="challenges" className="grid gap-10 mb-10 mx-6 md:grid-cols-2 lg:grid-cols-3 max-w-7xl mx-auto px-4">
          ...
          >
            <div className="relative h-56">
              <img
                ...
                onError={(e) => (e.target.src = "/challenges-placeholder.jpg")}
              />
            ...
            <div className="p-6 space-y-3">
              <h2 className="text-2xl font-bold text-gray-900">{ch.name}</h2>
              <p className="text-gray-700 text-sm line-clamp-2">{ch.description}</p>

              <div className="flex items-center gap-2 text-gray-700 text-sm">
                <CalendarDays size={16} className="text-[#E3002A]" />
                {ch.duration}
              </div>
              ...
              <motion.button
                ...
                className="w-full py-3 mt-4 rounded-xl font-semibold text-white text-lg shadow-md transition-all duration-300 bg-[#E3002A] hover:bg-red-700 hover:shadow-lg"
              >
                {ch.free ? "Start Free" : "Add to Cart"}
              </motion.button>
            </div>
          </motion.div>
        ))}
      ...
      {selectedChallenge && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 relative overflow-y-auto max-h-[90vh]">
            ...
            <img
              src={selectedChallenge.images[0]}
            ...
            <div className="flex flex-wrap gap-3 text-gray-600 text-sm mb-6">
              <span className="flex items-center gap-1">
                <User size={16} /> {selectedChallenge.trainer}
              </span>
              <span className="flex items-center gap-1">
                <CalendarDays size={16} /> {selectedChallenge.duration}
              </span>
              <span className="flex items-center gap-1">
                Difficulty: {selectedChallenge.difficulty}
              </span>
              {selectedChallenge.categories && (
                <span>Category: {selectedChallenge.categories}</span>
              )}
            </div>
            ...
            <button
              onClick={() => {
                handleAddToCart(selectedChallenge);
                setSelectedChallenge(null);
              }}
              className="w-full py-3 rounded-xl font-semibold text-white text-lg shadow-md transition-all duration-300 bg-[#E3002A] hover:bg-red-700 hover:shadow-lg"
            >
              {selectedChallenge.free ? "Start Free" : "Add to Cart"}
            </button>
          </div>
        </div>
      )}
...
export default ChallengesPage;a