    ...
    <div className="min-h-screen mb-30">
      ...
      <div className="flex justify-center gap-4 mt-12 mb-10">
        {["all", "included", "paid"].map((f) => (
          ...
          >
            {f === "included" ? "Included" : f === "paid" ? "Paid" : "All"}
            {filter === "included" && ` (${memberProgram.nutritionPlans.length})`}
          </button>
        ))}
      </div>
              ...
              <div className="relative h-56">
                ...
                <span
                  className={`absolute top-4 left-4 text-white text-sm font-semibold px-4 py-1.5 rounded-full shadow-md ${
                    isIncluded(tip) 
                      ? "bg-green-600" 
                      : tip.price === 0 || tip.free 
                        ? "bg-blue-600" 
                        : "bg-[#E3002A]"
                  }`}
                >
                  {isIncluded(tip) ? "Included" : tip.price === 0 || tip.free ? "Free" : `₹${tip.price}`}
                </span>
              </div>
