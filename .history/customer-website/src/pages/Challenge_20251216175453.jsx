...
const ChallengesPage = () => {
    ...
    const isIncluded = (ch) => ch.includedInProgram;
    ...
    return (
        <div className="min-h-screen ">
            ...
            {/* Challenges Grid */}
            <div id=""className="grid gap-10 mb-10 mx-6 md:grid-cols-2 lg:grid-cols-3">
                {currentChallenges.map((ch) => (
                    ...
                    >
                        <div className="relative h-56">
                            ...
                            <span
                                className={`absolute top-4 left-4 text-white text-20 font-semibold px-4 py-1.5 rounded-full shadow-md ${isIncluded(ch) ? "bg-green-600" : ch.free ? "bg-blue-600" : "bg-[#E3002A]"
                                    }`}
                            >
                                {isIncluded(ch) ? "Included" : ch.free ? "Free" : `₹${ch.price}`}
                            </span>
                        </div>
                        <div className="p-6 space-y-3">
                            ...
                            <motion.button
                                ...
                                disabled={isIncluded(ch)}
                                className={`w-full py-3 mt-4 rounded-xl font-semibold text-white text-lg shadow-md transition-all duration-300 ${isIncluded(ch)
                                    ? "bg-gray-400 cursor-not-allowed"
                                    : "bg-[#E3002A] hover:bg-red-700 hover:shadow-lg"
                                    }`}
                            >
                                {isIncluded(ch) ? "Included" : "Add to Cart"}
                            </motion.button>
                        </div>
                    </motion.div>
                ))}
