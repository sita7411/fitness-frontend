...
useEffect(() => {
  const fetchData = async () => {
    if (!selectedPlanId) return; // Wait until plan is selected
    ...
    try {
      const [plansRes, todayStatsRes, weeklyRes, userRes] = await Promise.all([
        ...
        axios.get("http://localhost:5000/api/users/me", { withCredentials: true }), // Yeh endpoint user return karta hoga
      ]);
      ...
      // ====== PERMANENT COMPLETED MEALS FROM USER PROGRESS ======
      const userProgress = userRes.data.nutritionProgress || [];
      const currentPlanProgress = userProgress.find(
        p => p.planId.toString() === selectedPlanId
      );

      if (currentPlanProgress) {
        const completedMealIds = currentPlanProgress.completedMeals.map(cm => cm.mealId.toString());
        setCompletedMeals(completedMealIds);
      } else {
        setCompletedMeals([]); // No progress yet for this plan
      }
...
}, [selectedPlanId]); // ← Re-run when plan changes
// ========================================================
