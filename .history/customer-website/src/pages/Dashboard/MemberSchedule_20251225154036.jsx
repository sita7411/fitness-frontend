...
import { useUserAuth } from "../context/UserAuthContext"; // ← अपने path के according adjust करें
...
export default function ProfessionalSchedule() {
  ...
  const handleSave = async () => {
    ...
    if (!user?._id) {
      alert("User not loaded. Please refresh or login again.");
      return;
    }

    try {
      if (selectedEvent) {
        // EDIT: Only manual events can be edited
        ...
        const payload = {
          ...
          date: format(selectedDate, "yyyy-MM-dd"),
          userId: user._id,         // ← Critical: backend needs this
          source: "manual",         // ← Explicitly set
          completed: false,
        };
      ...
      if (err.response?.status === 403) {
        alert("Not authorized. Only manual workouts can be added/edited.");
                          ...
                          <div className="flex flex-col truncate">
                            ...
                            <div className="mt-1">
                              <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                                ev.source === "program" ? "bg-blue-100 text-blue-800" :
