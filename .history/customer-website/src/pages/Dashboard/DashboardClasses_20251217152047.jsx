...
export default function MyClasses() {
  ...
  // Advanced Features (same as MyWorkouts)
  const [achievements, setAchievements] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem('classAchievements') || '[]');
    } catch { return []; }
  });
  const [streak, setStreak] = useState(() => parseInt(localStorage.getItem('classStreak') || '0', 10));
  const [lastWorkoutDate, setLastWorkoutDate] = useState(() => localStorage.getItem('classLastDate') || null);
  ...
  const completedInCurrent = currentProgram.days?.reduce((acc, d) =>
    acc + d.exercises.filter(ex => completedExercises.includes(ex.id)).length, 0) || 0;
  ...
  useEffect(() => {
    const loadClasses = async () => {
      ...
      try {
        ...
        const transformed = (res.data.classes || []).map(cls => ({
          ...
          days: (cls.days || []).map((day, i) => ({
            ...
            exercises: (day.exercises || []).map(ex => ({
              id: ex._id || `ex_${Math.random().toString(36)}`,
        ...
        if (transformed.length > 0) setSelectedProgramId(transformed[0].id);

        const saved = JSON.parse(localStorage.getItem('classProgress') || '[]');
        const allIds = transformed.flatMap(p => p.days.flatMap(d => d.exercises.map(e => e.id)));
        const validCompleted = saved.filter(id => allIds.includes(id));
        setCompletedExercises(validCompleted);
  ...
  useEffect(() => {
    ...
    saved.forEach(id => {
      const entry = JSON.parse(localStorage.getItem('classProgressDetailed') || '[]').find(p => p.exerciseId === id);
      if (entry) {
        const key = new Date(entry.timestamp).toDateString();
        if (key in map) map[key]++;
      }
    });
    setWeeklyProgress(Object.entries(map).map(([day, count]) => ({ day, count })));
  }, [completedExercises]);
  ...
  function finishExercise(manual = false) {
    ...
    const detailed = JSON.parse(localStorage.getItem('classProgressDetailed') || '[]');
    const newEntry = { exerciseId: ex.id, timestamp: Date.now(), date: today };
    const updatedDetailed = [...detailed.filter(p => p.exerciseId !== ex.id), newEntry];
    localStorage.setItem('classProgressDetailed', JSON.stringify(updatedDetailed));

    const newCompleted = Array.from(new Set([...completedExercises, ex.id]));
    setCompletedExercises(newCompleted);
    localStorage.setItem('classProgress', JSON.stringify(newCompleted));

    // Streak
    const prevLast = localStorage.getItem('classLastDate');
    if (!prevLast || prevLast !== today) {
      const yesterday = new Date(); yesterday.setDate(yesterday.getDate() - 1);
      const prevStreak = parseInt(localStorage.getItem('classStreak') || '0', 10);
      const newStreak = (prevLast === yesterday.toDateString()) ? prevStreak + 1 : 1;
      localStorage.setItem('classStreak', String(newStreak));
      localStorage.setItem('classLastDate', today);
      setStreak(newStreak);
      setLastWorkoutDate(today);
    }

    // Achievements
    const achSet = new Set(achievements);
    if (!achSet.has('first_ex')) achSet.add('first_ex');
    if (newCompleted.length >= 10) achSet.add('ten_ex');
    if (parseInt(localStorage.getItem('classStreak') || '0') >= 7) achSet.add('seven_day_streak');
    const arr = Array.from(achSet);
    localStorage.setItem('classAchievements', JSON.stringify(arr));
    setAchievements(arr);

    // Confetti
    const completedBeforeThis = currentDay.exercises.filter(ex =>
  ...
  function repeatDay() {
    ...
    const filtered = completedExercises.filter(id => !exIds.includes(id));
    setCompletedExercises(filtered);
    localStorage.setItem('classProgress', JSON.stringify(filtered));
    localStorage.setItem('classProgressDetailed', JSON.stringify(
      JSON.parse(localStorage.getItem('classProgressDetailed') || '[]').filter(p => !exIds.includes(p.exerciseId))
    ));
    setCurrentIndex(0);
  }

  function restartProgram() {
    const allIds = currentProgram.days.flatMap(d => d.exercises.map(e => e.id));
    const filtered = completedExercises.filter(id => !allIds.includes(id));
    setCompletedExercises(filtered);
    localStorage.setItem('classProgress', JSON.stringify(filtered));
    localStorage.setItem('classProgressDetailed', JSON.stringify(
      JSON.parse(localStorage.getItem('classProgressDetailed') || '[]').filter(p => !allIds.includes(p.exerciseId))
    ));
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <IconButton onClick={() => { const v = !reminderEnabled; setReminderEnabled(v); localStorage.setItem('classReminderEnabled', v); if (v && Notification.permission !== 'granted') Notification.requestPermission(); }}
        ...
        {classes.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-3">
            {classes.map(p => (
              <ProgramCard key={p.id} program={p} active={p.id === selectedProgramId}
                onClick={() => { setSelectedProgramId(p.id); setSelectedDayIndex(0); setCurrentIndex(0); setQuery(''); setFilter('All'); }}
              />
            ))}
          </div>
        )}
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            {days.map((d, idx) => {
              const status = d.exercises.length === 0 ? 'rest' :
                d.exercises.every(ex => completedExercises.includes(ex.id)) ? 'completed' :
                  d.exercises.some(ex => completedExercises.includes(ex.id)) ? 'in-progress' : 'pending';
            ...
            {!isRestDay && (
              <>
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <button onClick={() => finishExercise(true)} className="py-3 rounded-full text-white font-medium" style={{ background: THEME }}>
                    Mark Complete
                  </button>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
              ...
              <div className="mt-8 space-y-3">
                {days.map((d, i) => {
                  const done = d.exercises.filter(ex => completedExercises.includes(ex.id)).length;
                ...
                <div className="flex gap-2 justify-center">
                  {weeklyProgress.map(w => (
                    <div key={w.day} className="flex flex-col items-center text-xs">
                      <div className="w-8 h-8 flex items-center justify-center rounded-md bg-gray-100">{w.count}</div>
              ...
              <div className="mt-6">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <button onClick={() => { const v = !reminderEnabled; setReminderEnabled(v); localStorage.setItem('classReminderEnabled', v); }} className={`px-3 py-1 rounded ${reminderEnabled ? 'bg-red-600 text-white' : 'bg-gray-200'}`}>
