...
function ProgramCard({ program, active, onClick }) {
  return (
    <motion.div
      ...
      className={`w-full rounded-2xl cursor-pointer bg-white shadow-md overflow-hidden transition-all ${
        active ? 'ring-2 ring-red-600 shadow-[0_0_25px_rgba(227,0,42,0.3)]' : ''
      }`}
    >
      <div className="relative h-48 w-full overflow-hidden rounded-2xl">
        ...
        <div className="absolute left-4 bottom-4 text-white">
          <div className="text-xs tracking-wide opacity-90">{program.subtitle}</div>
...
export default function MyWorkoutsProPremium() {
  ...
  const totalExercisesInCurrentProgram =
    currentProgram.days?.reduce((acc, day) => acc + (day.exercises?.length || 0), 0) || 0;

  const completedInCurrentProgram =
    currentProgram.days?.reduce(
      (acc, day) =>
        acc + day.exercises.filter((ex) => completedExercises.includes(ex.id)).length,
      0
    ) || 0;

  const progressPercentage =
    totalExercisesInCurrentProgram === 0
      ? 0
  ...
  useEffect(() => {
    const loadPrograms = async () => {
      ...
      try {
        ...
        const transformed = res.data.programs.map((p, programIndex) => ({
          id: p.id || p._id,
          title: p.title,
          ...
          days: (p.days || []).map((d, dayIndex) => ({
            day: d.day || dayIndex + 1,
            title: d.title || `Day ${d.day || dayIndex + 1}`,
            exercises: (d.exercises || []).map((ex, exIndex) => ({
              id: ex.id || `ex_${programIndex}_${dayIndex}_${exIndex}`,
              title: ex.title,
              type: ex.type || 'reps',
              time: ex.time || 30,
              reps: ex.reps || 12,
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        console.error('Failed to load progress/streak', err);
      }
    };
  ...
  function getSecondsForExercise(ex) {
    ...
    if (ex.type === 'reps') return (ex.reps || 12) * 2; // 2 sec per rep estimate
    return 30;
  }
  ...
  async function finishExercise(manual = false) {
    ...
    const totalSeconds = currentDay.exercises.reduce((sum, e) => {
      return sum + (e.type === 'time' ? e.time || 0 : (e.reps || 12) * (e.sets || 3) * 3);
    }, 0);
    ...
    try {
      ...
      if (res.data.success && !res.data.alreadyCompleted) {
        ...
        // Sync workout minutes for this exercise
        const timeSpentSeconds = ex.type === 'time' ? ex.time : (ex.reps || 12) * (ex.sets || 3) * 3;
        const minutes = Math.round(timeSpentSeconds / 60);
        await axios
          .post(`${API_BASE}/stats/workout-minutes`, { minutes }, { withCredentials: true })
        ...
        const allCompleted = dayExercises.every(
          (e) => completedExercises.includes(e.id) || e.id === ex.id
        );
        ...
        if (allCompleted) {
          ...
          try {
            const completeDayRes = await axios.post(
            ...
            if (completeDayRes.data.streak !== undefined) {
              setStreak(completeDayRes.data.streak);
            }
          } catch (err) {
      ...
      alert('Could not save progress. Check connection.');
    }

    setRunning(false);

    // Auto advance
    const nextUnfinished = exercises.findIndex(
      (e, i) => i > currentIndex && !completedExercises.includes(e.id)
    );
    if (nextUnfinished !== -1) {
      setCurrentIndex(nextUnfinished);
      setTimeout(() => {
        const el = document.getElementById(`exercise-${exercises[nextUnfinished].id}`);
  ...
  async function repeatDay() {
    try {
      ...
      setCompletedExercises((prev) =>
        prev.filter((id) => !currentDay.exercises.map((e) => e.id).includes(id))
      );
      setCurrentIndex(0);
    } catch (err) {
      console.error('Failed to reset day', err);
      alert('Local reset only — will sync later.');
      setCompletedExercises((prev) =>
        prev.filter((id) => !currentDay.exercises.map((e) => e.id).includes(id))
      );
      setCurrentIndex(0);
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Premium, focused workout hub — track programs and progress.
            </p>
            ...
            <div className="mt-2 flex items-center gap-4 text-sm text-gray-600">
              ...
              <div className="flex items-center gap-2">
                ...
                <span>
                  Today: <strong>{backendWorkoutMinutes}</strong> min
                </span>
              </div>
              <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <IconButton onClick={() => setReminderEnabled((v) => !v)}>
              <Bell size={16} color={reminderEnabled ? THEME : '#999'} />
            </IconButton>

            <IconButton onClick={repeatDay} disabled={!currentProgram.id}>
              <Repeat size={16} color={THEME} />
            </IconButton>

            <IconButton onClick={restartProgram} disabled={!currentProgram.id}>
            ...
            <div className="md:col-span-2">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
                <div className="p-3 rounded-lg flex items-center gap-2 border border-gray-100 shadow-sm">
                  <Clock size={16} color={THEME} /> {currentProgram.duration}
                </div>
                <div className="p-3 rounded-lg flex items-center gap-2 border border-gray-100 shadow-sm">
                  <Zap size={16} color={THEME} /> {currentProgram.calories}
                </div>
                <div className="p-3 rounded-lg flex items-center gap-2 border border-gray-100 shadow-sm">
                  <Calendar size={16} color={THEME} /> {currentProgram.totalDays} days
                </div>
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                {currentProgram.equipment?.length > 0 ? (
                  currentProgram.equipment.map((eq, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1 rounded-full bg-gray-50 text-sm text-gray-600 shadow-sm"
                    >
                      {eq}
                    </span>
                  ))
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="w-full">
                ...
                <div className="text-sm text-gray-500 mt-2 text-center">
                  {completedInCurrentProgram} / {totalExercisesInCurrentProgram} exercises
                </div>
              </div>
              <button
            ...
            {days.map((d, idx) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-full shadow-sm border ${bg} whitespace-nowrap`}
                >
                  Day {d.day}
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <input
            ...
            className="px-4 py-2 rounded-full border border-gray-200 w-full md:w-80 focus:outline-none focus:border-red-600"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          <main
            ref={exerciseListRef}
            className="md:col-span-5 bg-white rounded-2xl p-6 shadow-sm max-h-[640px] overflow-y-auto border border-gray-100"
          >
            ...
            <div className="space-y-3">
              {displayExercises.length === 0 ? (
                <div className="py-12 text-center text-gray-500">
                  No exercises — enjoy your rest day! 🌴
                </div>
              ) : (
                displayExercises.map((ex, idx) => (
                  <motion.div
                    ...
                    className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all ${
                      exercises[currentIndex]?.id === ex.id
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-20 h-20 rounded-xl overflow-hidden shadow-sm flex-shrink-0">
                        <img
                          src={ex.thumbnail || '/trainer-3.jpg'}
                          alt={ex.title}
                          className="w-full h-full object-cover"
                        />
                      </div>
                      <div>
                        <div className="font-medium text-gray-800 flex items-center gap-2">
                          ...
                          {completedExercises.includes(ex.id) && (
                            <CheckCircle size={18} color={THEME} />
                          )}
                        </div>
                        <div className="text-sm text-gray-500">
                          {ex.type === 'time'
                            ? `${ex.time}s`
                            : `${ex.reps} reps × ${ex.sets || 1} sets`}
                        </div>
                        {ex.description && (
          ...
          <aside className="md:col-span-7 bg-white rounded-2xl p-6 shadow-sm flex flex-col gap-6 border border-gray-100">
            <div className="text-center">
              ...
              <div className="text-6xl font-bold mt-4" style={{ color: THEME }}>
                {String(minutes).padStart(2, '0')}:{String(secs).padStart(2, '0')}
              </div>
              <div className="text-xl font-semibold text-gray-800 mt-4">
                {exercises[currentIndex]?.title || 'Select an exercise to begin'}
              </div>
              <div className="text-sm text-gray-500 mt-2 max-w-md mx-auto">
                {exercises[currentIndex]?.description || 'Choose from the list on the left'}
              </div>
            </div>

            {isRestDay ? (
              <div className="rounded-2xl bg-gradient-to-br from-green-50 to-blue-50 h-96 flex items-center justify-center flex-col shadow-inner">
                <h2 className="text-3xl font-bold text-gray-700">Rest Day</h2>
                <p className="text-gray-600 mt-3 text-center px-8">
                  Take time to recover. Your body needs it to grow stronger! 💪
                </p>
              </div>
            ) : (
              <div className="rounded-2xl bg-gray-50 h-96 overflow-hidden shadow-inner">
                <img
            ...
            {!isRestDay && exercises.length > 0 && (
              <>
                <div className="grid grid-cols-3 gap-3">
                  <button
                    onClick={togglePlay}
                    className={`py-4 rounded-full flex items-center justify-center gap-2 font-semibold text-lg transition ${
                      running
                  ...
                  <button
                    ...
                    className={`py-4 rounded-full font-semibold text-lg transition shadow-md ${
                      completedExercises.includes(exercises[currentIndex]?.id)
                        ? 'bg-gray-400 text-white cursor-not-allowed'
                        : 'bg-red-600 text-white hover:opacity-90'
                    }`}
                    style={{
                      background: completedExercises.includes(exercises[currentIndex]?.id)
                        ? '#94a3b8'
                        : THEME,
                    }}
                  >
                  ...
                  <button
                    ...
                    className="py-4 rounded-full bg-yellow-500 text-white font-semibold hover:bg-yellow-600 transition"
                  >
                    Skip
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="py-3 rounded-full bg-gray-100 text-gray-700 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Previous
                  </button>
                  <button
                    onClick={() => {
                      const nextDay = selectedDayIndex + 1;
      ...
      <AnimatePresence>
        {showCompletePopup && (
          ...
          >
            ...
            >
              <div className="absolute top-0 left-0 w-full h-2" style={{ backgroundColor: THEME }} />

              <h2 className="text-3xl font-bold text-gray-900 mt-4 text-center">
                Day {currentDay.day} Completed! 🎉
              </h2>

              <p className="text-gray-600 mt-3 text-center mb-8">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-4 border border-gray-300 rounded-xl text-center text-lg focus:border-red-600 focus:ring-4 focus:ring-red-100 transition"
                    min="30"
                    max="250"
                  />
                </div>
              ...
              <div className="flex gap-4 mt-8">
                <button
                  ...
                  className="flex-1 py-4 rounded-xl border-2 border-gray-300 text-gray-700 font-medium hover:bg-gray-50 transition"
                >
                  Skip
                </button>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      // Already called complete-day in finishExercise
                      console.log('Extra data saved successfully');
                    } catch (error) {
                      console.error('Failed to save extra data:', error);
                      alert('Main progress saved. Extra stats will sync later.');
                    } finally {
                      setHeartRate('');
                      setWeightInput('');
                      setShowCompletePopup(false);
                  ...
                  className="flex-1 py-4 rounded-xl text-white font-bold text-lg shadow-lg transition hover:opacity-90"
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setShowProgress(false)} className="text-gray-500 hover:text-gray-700">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-gray-600 mt-4 text-lg">
                  {completedInCurrentProgram} of {totalExercisesInCurrentProgram} exercises completed
                </p>
              </div>
                ...
                {days.map((d, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={completed === total ? 'text-green-600 font-semibold' : 'text-gray-600'}
                      >
