...
export default function MyWorkouts() {
  ...
  // ADD THIS useEffect — Achievements load करने के लिए
  useEffect(() => {
    const loadAchievements = async () => {
      try {
        const res = await axios.get(`${API_BASE}/achievements`, { withCredentials: true });
    ...
    loadAchievements();
  }, []); // empty dependency → सिर्फ एक बार app load पर call होगा
  // Load progress, streak, and weekly stats when program changes
  useEffect(() => {
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      // Update streak from response if provided
                      // (assuming backend returns updated streak)
                    } catch (error) {
