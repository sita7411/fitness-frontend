...
export default function NutritionPage() {
  ...
  // ✅ Sab derived values ek saath, sahi order mein — NO TDZ ERROR
  const filteredMeals = filter === "All"
    ? currentDay.meals || []
    : (currentDay.meals || []).filter((m) => 
        m.type?.toLowerCase() === filter.toLowerCase()
      );

  const selectedMeal = currentDay.meals?.find((m) => m._id === selectedMealId) 
    || filteredMeals[0] 
    || null;

  const totalMeals = currentDay.meals?.length || 1;
  
  // ✅ Sirf current day ke completed meals count karo
