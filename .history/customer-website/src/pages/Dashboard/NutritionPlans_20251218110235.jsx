...
function MealListItem({ meal, onSelect, selected, completed, onToggleComplete }) {
  return (
    <motion.div
      ...
      className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer border transition-shadow hover:shadow-md ${
        selected ? "ring-2 ring-red-500 bg-red-50" : "bg-white border-gray-100"
      }`}
    >
      <img
      ...
      <div className="flex flex-col items-end gap-2">
        <button
          ...
          className={`px-3 py-1 rounded-full text-xs transition-colors ${
            completed ? "bg-green-600 text-white" : "bg-white border text-gray-600"
          }`}
        >
          {completed ? (
      ...
      <div className="mt-3 flex items-center gap-2">
        <div className="flex-1 grid grid-cols-8 gap-2">
          {Array.from({ length: cupsTotal }).map((_, i) => (
            <div
              key={i}
              className={`h-10 rounded-md border flex flex-col items-center justify-center transition ${
                i < filled ? "bg-blue-100 border-blue-200" : "bg-gray-50 border-gray-200"
              }`}
            >
              <Droplet
                size={16}
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCompletedMeals(statsRes.data.completedMeals || []); // Critical fix
      } catch (err) {
        console.error(err);
          ...
          <div className="grid grid-cols-3 gap-4 pb-3">
            {plans.map((plan) => (
              <div
                ...
                className={`flex-shrink-0 w-64 rounded-2xl cursor-pointer overflow-hidden border transition-shadow ${
                  plan._id === selectedPlanId ? "ring-2 ring-red-600 shadow-lg" : "border-gray-100"
                }`}
              >
            ...
            {days.map((d, idx) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-full shadow-sm border whitespace-nowrap transition ${
                    isSelected
            ...
            {["All", "Breakfast", "Lunch", "Dinner", "Snack"].map((f) => (
              <button
                ...
                className={`px-4 py-2 rounded-full border text-sm transition ${
                  filter === f ? "bg-red-600 text-white" : "bg-white text-gray-700"
                }`}
