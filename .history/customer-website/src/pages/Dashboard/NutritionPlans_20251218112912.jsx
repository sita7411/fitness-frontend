...
export default function NutritionPage() {
  ...
  // Auto set timer when selected meal changes
  useEffect(() => {
    if (!selectedMealId || !plans.length) return;

    const plan = plans.find((p) => p._id === selectedPlanId);
    const day = plan?.days?.[selectedDayIndex];
    const meal = day?.meals?.find((m) => m._id === selectedMealId);
    if (!meal) return;

    const prep = extractMinutes(meal.prepTime);
    const cook = extractMinutes(meal.cookTime);
    const total = prep + cook || 15;

    setTimeLeft(total * 60);
    setTimerRunning(false);
  }, [selectedMealId, selectedDayIndex, selectedPlanId, plans]);
                    ...
                    <div className="lg:col-span-2 space-y-4">
                      <div className="bg-white border rounded-2xl p-4 shadow-sm">
                        ...
                        <div className="flex justify-center gap-3">
                          ...
                          <button
                            onClick={() => {
                              const prep = extractMinutes(selectedMeal?.prepTime);
                              const cook = extractMinutes(selectedMeal?.cookTime);
                              setTimeLeft((prep + cook || 15) * 60);
                              setTimerRunning(false);
                            }}
