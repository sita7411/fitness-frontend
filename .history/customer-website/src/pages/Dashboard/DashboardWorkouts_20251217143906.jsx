...
export default function MyWorkoutsProPremium() {
  ...
  async function finishExercise(manual = false) {
    ...
    try {
      ...
      if (res.data.success && !res.data.alreadyCompleted) {
        ...
        if (allCompleted) {
          ...
          try {
    const res = await axios.post(`${API_BASE}/programs/${currentProgram.id}/complete-day`, {
      day: currentDay.day,
    ...
    if (res.data.streak !== undefined) {
      ...
      setLastWorkoutDate(new Date().toDateString()); // optional, for fallback
    }
  } catch (err) {
    console.error("Failed to update day completion with streak", err);
  }
}
        }
      }
