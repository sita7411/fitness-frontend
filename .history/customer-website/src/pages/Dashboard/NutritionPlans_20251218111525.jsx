// src/pages/NutritionPage.jsx → FINAL FIXED VERSION (SAME UI/DESIGN, FULLY WORKING)
...
export default function NutritionPage() {
  ...
  const completeMeal = async (meal) => {
    if (!meal?.nutrition) {
      toast.error("Nutrition info missing.");
      return;
    }

    if (completedMeals.includes(meal._id)) {
      toast.info("Meal already added for today.");
      return;
    }
    ...
    try {
      ...
      toast.success("Meal added to today's stats!");
    } catch (err) {
      console.error(err);
  ...
  // Load plans + today's stats (water + completed meals)
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Today stats — now correctly using todayStatsRes
        setCups(todayStatsRes.data.water || 0);
        setCompletedMeals(todayStatsRes.data.completedMeals || []);

        // Weekly calories from backend
        const weeklyData = weeklyRes.data;
        const caloriesArray = weeklyData.map(item => item.calories || 0);
  ...
  const handleDownloadTxt = () => {
    ...
    a.href = url;
    a.download = `grocery-day-${currentDay.day || selectedDayIndex + 1}.txt`;
    a.click();
  ...
  const handleDownloadCsv = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `grocery-day-${currentDay.day || selectedDayIndex + 1}.csv`;
    a.click();
    URL.revokeObjectURL(url);
      ...
      <div className="min-h-screen bg-white rounded-lg p-4 md:p-8">
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 mt-2">
              ...
              <div className="bg-white rounded-2xl p-6 shadow border border-gray-100">
                ...
                <div className="mt-4 flex items-center justify-center">
                  <svg width="150" height="150" viewBox="0 0 120 120">
                    ...
                    <circle
                      ...
                      strokeWidth="10"
                      fill="none"
                      strokeDasharray={`${(completedMeals.length / (currentDay.meals?.length || 1)) * 314} 314`}
                      strokeLinecap="round"
                ...
                <div className="text-center -mt-2">
                  <p className="text-3xl font-bold text-gray-900">
                    {Math.round((completedMeals.length / (currentDay.meals?.length || 1)) * 100)}%
                  </p>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-semibold">
                      Day {currentDay.day || selectedDayIndex + 1} • {currentDay.title || "Daily Meals"}
                    </h3>
            ...
            {/* Right */}
            {/* Right */}
<div className="md:col-span-7 bg-white rounded-2xl shadow border border-gray-100 overflow-hidden">
  {selectedMeal ? (
    <div className="flex flex-col h-full">
      ...
      <div className="relative h-64 md:h-80 flex-shrink-0">
        ...
        <div className="absolute bottom-6 left-6 text-white max-w-lg">
          <span className="inline-block px-4 py-1.5 bg-white/20 backdrop-blur rounded-full text-sm font-medium">
            {selectedMeal.type}
          </span>
          ...
          {selectedMeal.description && (
            <p className="text-sm md:text-base text-white/90 mt-2 line-clamp-2">
              {selectedMeal.description}
            </p>
          )}
        </div>
      ...
      <div className="p-6 md:p-8 flex-1 flex flex-col">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
          ...
          <div className="flex flex-wrap items-center gap-5 text-gray-600">
            <div className="flex items-center gap-2">
              <Clock size={18} />
              <span className="font-medium">{selectedMeal.prepTime || 15} min prep</span>
            </div>
            <div className="flex items-center gap-2">
              <Utensils size={18} />
              <span className="font-medium">{selectedMeal.cookTime || 30} min cook</span>
            </div>
            <div className="flex items-center gap-2">
              <Flame size={18} className={selectedMeal.difficulty?.toLowerCase() === "easy" ? "text-green-600" : selectedMeal.difficulty?.toLowerCase() === "hard" ? "text-red-600" : "text-orange-600"} />
              <span className="font-medium capitalize">{selectedMeal.difficulty || "Easy"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 flex-1">
              ...
              <ul className="space-y-3">
                {selectedMeal.ingredients?.map((ing, i) => (
                  <li key={i} className="flex items-start gap-3 text-gray-700">
                    <div className="w-5 h-5 rounded-full bg-white shadow-sm flex items-center justify-center mt-0.5">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6 border">
              <h5 className="font-bold text-lg mb-4">Nutrition Facts</h5>
              <div className="space-y-4 text-gray-800">
                <div className="flex justify-between items-center py-2 border-b">
                  <span className="font-medium">Calories</span>
                  <span className="text-xl font-bold">{selectedMeal.nutrition?.calories || 0}</span>
                </div>
                <div className="flex justify-between py-2 border-b">
                  <span>Protein</span>
                  <span className="font-bold text-green-600">{selectedMeal.nutrition?.protein || 0}g</span>
                </div>
                <div className="flex justify-between py-2 border-b">
                  <span>Carbohydrates</span>
                  <span className="font-bold text-orange-600">{selectedMeal.nutrition?.carbs || 0}g</span>
                </div>
                <div className="flex justify-between py-2">
                  <span>Total Fat</span>
                  <span className="font-bold text-purple-600">{selectedMeal.nutrition?.fat || 0}g</span>
                </div>
              </div>
            </div>

            {/* Tools */}
            {selectedMeal.tools && selectedMeal.tools.length > 0 && (
              <div className="bg-gray-50 rounded-2xl p-6">
                <h5 className="font-bold text-lg mb-4">Tools Needed</h5>
                <ul className="space-y-2 text-gray-700">
                  {selectedMeal.tools.map((tool, i) => (
            ...
            {selectedMeal.notes && selectedMeal.notes.length > 0 && (
              <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6">
                <h5 className="font-bold text-lg mb-4 text-blue-900 flex items-center gap-2">
                  <Info size={20} />
                  Pro Tips
                </h5>
                <ul className="space-y-3 text-blue-800">
                  {selectedMeal.notes.map((note, i) => (
                    <li key={i} className="flex gap-3">
                      <Star size={16} className="text-blue-600 flex-shrink-0 mt-1" />
                      <span>{note}</span>
                    </li>
                  ))}
                </ul>
        ...
        <div className="mt-8 pt-6 border-t flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-4">
            <button
              ...
              className={`px-6 py-3 rounded-full font-bold text-white transition shadow-lg ${
                completedMeals.includes(selectedMeal._id)
            ...
            <button className="px-5 py-3 rounded-full border-2 border-gray-300 font-medium hover:bg-gray-50 transition">
              Save Recipe
            </button>

            <button className="px-5 py-3 rounded-full bg-gray-100 font-medium hover:bg-gray-200 transition hidden sm:flex items-center gap-2">
              <Flame size={18} />
              Start Cooking Timer
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 text-gray-600">
            ...
            <span className="text-sm">• 220 reviews</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="h-full flex flex-col items-center justify-center text-center py-20 px-8">
      ...
      <h2 className="text-3xl font-bold text-gray-800 mb-3">Rest Day</h2>
      <p className="text-lg text-gray-600 max-w-md">
        No meals planned for today. Take this time to recover, meal prep, or enjoy a flexible eating day!
      </p>
