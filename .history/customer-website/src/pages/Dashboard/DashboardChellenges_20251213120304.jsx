...
export default function DashboardChallenges() {
    ...
    const addTodayGoal = async () => {
        ...
        try {
            const res = await axios.post(`${API_BASE}/stats/today-goal`, {
    ...
    const toggleGoalComplete = async (goalId) => {
        try {
            const res = await axios.put(
                `${API_BASE}/stats/today-goal/toggle`,
                { goalId },
                { withCredentials: true }
            );
    ...
    const deleteGoal = async (goalId) => {
        try {
            const res = await axios.delete(`${API_BASE}/stats/today-goal`, {
                data: { goalId },
                withCredentials: true
            });
    ...
    // Same features as MyWorkouts
    const [achievements, setAchievements] = useState(() => {
        try {
            return JSON.parse(localStorage.getItem('challengeAchievements') || '[]');
        } catch {
            return [];
        }
    });
    const [streak, setStreak] = useState(() => parseInt(localStorage.getItem('challengeStreak') || '0', 10));
    const [lastWorkoutDate, setLastWorkoutDate] = useState(() => localStorage.getItem('challengeLastDate') || null);
    ...
    useEffect(() => {
        const loadChallenges = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`${API_BASE}/challenges/user/challenges`, { withCredentials: true });
    ...
    async function finishExercise(auto = false) {
        ...
        if (!completedExercises.includes(ex.id)) {
            ...
            // Extra safety: check if already exists
        ...
        // Ab sab variables ready hain → YAHAN DEBUG LOG DAALO
                                        ...
                                        <div className="p-6 space-y-6">
                                            ...
                                            {todayGoals.length > 0 && (
                                                <div className="mt-10 pt-8 border-t-2 border-gray-100">
                                                    ...
                                                    {/* Har Goal Ke Liye Individual Progress Bar */}
                                                    {/* Har Goal Ke Liye Individual Progress Bar – SIRF 0% YA 100% */}
