// src/pages/NutritionPage.jsx → FULL FINAL WORKING VERSION (NO CUT, SAME DESIGN)
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-end gap-2">
        ...
        >
          {completed ? (
            <span className="flex items-center gap-1"><CheckCircle size={14} />Done</span>
          ) : (
            "Mark"
          )}
        </button>
      ...
      <div className="mt-3 flex items-center gap-2">
        <div className="flex-1 grid grid-cols-8 gap-2">
          {Array.from({ length: cupsTotal }).map((_, i) => (
            <div
              key={i}
              className={`h-10 rounded-md border flex flex-col items-center justify-center transition ${i < filled
...
function WeeklyMacroSummary({ data }) {
  if (!data || !data.length) return <div className="text-center text-gray-400 py-4">No data</div>;
  ...
  return (
    <div className="bg-white border rounded-2xl p-4 shadow-sm w-full">
      ...
      <div className="mt-4 space-y-2">
        {data.map((d, i) => {
          const widthPercent = (d / max) * 100;
          return (
            <div key={i} className="flex items-center gap-3">
              <div className="w-6 text-xs text-gray-500">{["M", "T", "W", "T", "F", "S", "S"][i]}</div>
              <div className="flex-1 bg-gray-100 rounded-full h-4 overflow-hidden">
                <div
                  className="h-4 rounded-full"
                  style={{
                    width: `${widthPercent}%`,
                    backgroundColor: i === data.length - 1 ? "#e3002a" : "#f87171",
                  }}
                ></div>
              </div>
              <div className="w-10 text-right text-xs text-gray-600">{d}</div>
            </div>
          );
        })}
      </div>
...
export default function NutritionPage() {
  ...
  // FIXED: Complete meal function with access to state
  ...
  // Load plans + water from today's stats
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [plansRes, statsRes] = await Promise.all([
axios.get("http://localhost:5000/api/nutrition/my/plans", { withCredentials: true })          axios.get("http://localhost:5000/api/stats/today", { withCredentials: true })
        ]);
            ...
            {days.map((d, idx) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-full shadow-sm border whitespace-nowrap transition ${isSelected ? "bg-red-600 text-white" : status === "active" ? "bg-white text-gray-700" : "bg-gray-100 text-gray-500"
                    }`}
            ...
            <div className="md:col-span-5 flex flex-col gap-4">
              ...
              <div className="bg-white rounded-2xl p-6 shadow border border-gray-100">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-semibold">Day {currentDay.day || selectedDayIndex + 1} • {currentDay.title || "Daily Meals"}</h3>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <WeeklyMacroSummary data={[1800, 1900, 2000, 1700, 2200, 2100, 1600]} />
                ...
                <>
                  <div className="relative overflow-hidden rounded-2xl">
                    <div className="w-full h-64 md:h-80 object-cover rounded-2xl bg-gray-100 flex items-center justify-center text-gray-400">
                      <img src={selectedMeal.thumbnail || "/meal-1.jpg"} alt={selectedMeal.title} className="w-full h-full object-cover" />
                    </div>
                    ...
                    <div className="absolute left-6 bottom-6 text-white max-w-[75%]">
                      <div className="text-sm bg-white/10 px-3 py-1 rounded-full inline-flex items-center gap-2">
                        <span className="text-xs opacity-90">{selectedMeal.type}</span>
                      </div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-3 items-center">
                      <div className="flex items-center gap-2 text-sm text-gray-600 whitespace-nowrap">
                        <Clock size={16} /> <span className="font-medium">{selectedMeal.prepTime || "15 min"}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-gray-600 whitespace-nowrap">
                        <Calendar size={16} /> <span className="font-medium">{selectedMeal.cookTime || "30 min"}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm text-gray-600 whitespace-nowrap">
                        <Flame size={16} /> <span className="font-medium">{selectedMeal.difficulty || "Easy"}</span>
                      </div>
                    </div>
                  </div>
                        ...
                        <ul className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-gray-700">
                          {selectedMeal.ingredients?.map((ing, i) => (
                            <li key={i} className="flex items-start gap-3">
                              <div className="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center text-sm text-blue-600">•</div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-gray-50 border rounded-2xl p-2 shadow-sm">
                        ...
                        <div className="mt-3 text-sm text-gray-700 space-y-2">
                          <div className="flex justify-between"><span>Calories</span><span className="font-semibold">{selectedMeal.nutrition?.calories || 0} cal</span></div>
                          <div className="flex justify-between"><span>Protein</span><span className="font-semibold">{selectedMeal.nutrition?.protein || 0} g</span></div>
                          <div className="flex justify-between"><span>Carbohydrates</span><span className="font-semibold">{selectedMeal.nutrition?.carbs || 0} g</span></div>
                          <div className="flex justify-between"><span>Total Fat</span><span className="font-semibold">{selectedMeal.nutrition?.fat || 0} g</span></div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mt-2">
                    <div className="flex items-center gap-3">
                      ...
                      <button className="px-4 py-2 rounded-full border text-sm text-gray-700 hidden md:inline-flex">
                        Start Cook
                      </button>
                    </div>
