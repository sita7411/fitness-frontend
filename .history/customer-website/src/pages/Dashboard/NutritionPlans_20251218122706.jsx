...
export default function NutritionPage() {
 ...
 const getMinutes = (value) => {
  ...
  // Auto set timer when selected meal changesuseEffect(() => {
  if (!selectedMeal) return;

  const prep = selectedMeal.prepTime ?? DEFAULT_PREP_MIN;
  const cook = selectedMeal.cookTime ?? DEFAULT_COOK_MIN;
  const totalMinutes = (getMinutes(prep) + getMinutes(cook));
                      ...
                      <div className="flex items-center gap-2 whitespace-nowrap">
                        ...
                        <span>
                          {selectedMeal.totalTime ||
                            `${(extractMinutes(selectedMeal.prepTime) || 10) +
                            (extractMinutes(selectedMeal.cookTime) || 20)} min`}
                        </span>
                      </div>
