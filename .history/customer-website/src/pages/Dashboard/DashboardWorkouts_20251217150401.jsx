...
export default function MyWorkoutsProPremium() {
        ...
        const transformed = res.data.programs.map((p) => ({
          ...
          days: (p.days || []).map(d => ({
            ...
            exercises: (d.exercises || []).map(ex => ({
              ...
              reps: ex.reps || 12,
              sets: ex.sets || 3,
              thumbnail: ex.thumbnail ? `thumbnail: ex.thumbnail || '/trainer-3.jpg',://localhost:5000${ex.thumbnail.startsWith('/') ? '' : '/'}${ex.thumbnail}` : '/trainer-3.jpg',
  ...
  async function finishExercise(manual = false) {
    ...
    // Define variables यहाँ (popup values + totalSeconds)
      ...
      if (res.data.success && !res.data.alreadyCompleted) {
        ...
        if (allCompleted) {
          ...
          try {
            ...
            console.error("Failed to complete day with streak", err);
          }
        }
      }
    } catch (err) {
  ...
  async function repeatDay() {
    try {
      ...
      console.error("Failed to reset day on server", err);
      alert("Local reset only — will sync later.");
      // Still do local reset
      setCompletedExercises(prev => prev.filter(id => !currentDay.exercises.map(e => e.id).includes(id)));
      setCurrentIndex(0);
    }
  }
  ...
  async function restartProgram() {
    try {
      ...
      console.error("Failed to restart program on server", err);
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <IconButton
              onClick={repeatDay}
              disabled={!currentProgram.id}
              className={`flex items-center gap-2 bg-white border border-gray-100 shadow-sm ${!currentProgram.id ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              ...
              const bg =
                selectedDayIndex === idx
                  ...
                  : status === 'completed'
                    ...
                    : status === 'in-progress'
                      ? 'bg-orange-100 text-orange-800 border-orange-300'  // नया
