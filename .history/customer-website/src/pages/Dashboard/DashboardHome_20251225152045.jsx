...
export default function DashboardHome() {
  ...
  const [activityLoading, setActivityLoading] = useState(true); // ← NEW
  ...
  // Fetch all dashboard data including recent activity
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [weeklyRes, todayRes, sessionRes, scheduleRes, activityRes] = await Promise.all([
          ...
          axios.get("/api/activity/recent", { withCredentials: true }), // ← NEW API
        ]);
        ...
        // Recent Activity - NEW
        if (activityRes.data?.success && Array.isArray(activityRes.data.activities)) {
  ...
  return (
    <div className="p-5 bg-white w-full rounded-lg">
      <div className="max-w-6xl mx-auto">
        ...
        {/* CALORIES NUTRITION + POPULAR COURSE + SCHEDULE */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-10">
                ...
                {todaySchedule.map((event, idx) => (
                  <div key={idx} className="flex items-center gap-4 hover:bg-gray-50 -mx-3 px-3 py-3 rounded-xl transition-all duration-200">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${event.completed ? "bg-green-100" : "bg-[#e3002a]/10"}`}>
                      ...
                      {(!["yoga", "cardio", "strength", "gym", "lift"].some((k) => event.title.toLowerCase().includes(k)) && !event.source) && (
                        <Calendar size={22} className={event.completed ? "text-green-600" : "text-[#e3002a]"} />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`font-semibold text-gray-900 truncate pr-6 ${event.completed ? "text-gray-400" : ""}`}>
                        {event.title}
                      </p>
        ...
        {/* BOTTOM ROW: Active Session + Recent Activity */}
        <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_0.8fr] gap-6 mt-8">
          ...
          {/* Recent Activity - NOW FULLY LIVE */}
                ...
                {recentActivity.map((item, idx) => (
                  <div key={idx} className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#E3002A]/10 flex items-center justify-center flex-shrink-0">
                      {item.icon === "Dumbbell" && <Dumbbell size={20} className="text-[#E3002A]" />}
                      {item.icon === "Droplet" && <Droplet size={20} className="text-[#E3002A]" />}
                      {item.icon === "Footprints" && <Footprints size={20} className="text-[#E3002A]" />}
                      {item.icon === "StretchHorizontal" && <StretchHorizontal size={20} className="text-[#E3002A]" />}
                      {item.icon === "Flame" && <Flame size={20} className="text-[#E3002A]" />}
                      {item.icon === "Activity" && <Activity size={20} className="text-[#E3002A]" />}
                      {item.icon === "HeartPulse" && <HeartPulse size={20} className="text-[#E3002A]" />}
                      {item.icon === "Target" && <Target size={20} className="text-[#E3002A]" />}
                      {(!item.icon || !["Dumbbell", "Droplet", "Footprints", "StretchHorizontal", "Flame", "Activity", "HeartPulse", "Target"].includes(item.icon)) && (
                        <Zap size={20} className="text-[#E3002A]" />
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-gray-900 font-semibold truncate">{item.text}</p>
                      <span className="text-gray-500 text-xs">
                        {item.time || (item.timestamp ? new Date(item.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "Just now")}
                      </span>
                    </div>
                  </div>
                ))}
