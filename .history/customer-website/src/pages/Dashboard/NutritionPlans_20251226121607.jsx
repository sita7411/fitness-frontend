...
export default function NutritionPage() {
  ...
  const completeMeal = async (meal) => {
    ...
    if (completedMeals.includes(meal._id.toString())) {
    ...
    try {
      ...
      setCompletedMeals((prev) =>
        prev.includes(meal._id.toString())
          ? prev
          : [...prev, meal._id.toString()]
      );
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (userPlans.length > 0) {
          ...
          // Progress yahan nahi load karo — loadProgress useEffect karega
        }
  ...
  useEffect(() => {
    ...
    const loadProgress = async () => {
      try {
        setProgressLoading(true);  // ← Start loading
        const userRes = await axios.get("http://localhost:5000/api/auth/me", { withCredentials: true });
            console.log("FULL USER:", userRes.data.user);
      console.log("PROGRESS:", userRes.data.user?.nutritionProgress);
        ...
        console.error("Progress reload error:", err);
      } finally {
        setProgressLoading(false);  // ← End loading
      }
    };
                    ...
                    <div key={meal._id} className="flex justify-between items-start">
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{
                            background: completedMeals.includes(selectedMeal._id.toString())
                              ? "#16a34a"
                              : THEME,
                          }}
