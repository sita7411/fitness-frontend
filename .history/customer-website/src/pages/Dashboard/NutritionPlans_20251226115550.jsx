...
export default function NutritionPage() {
  ...
  const [weeklyCalories, setWeekconst [progressLoading, setProgressLoading] = useState(true);  // ← Yeh add karolyCalories] = useState([0, 0, 0, 0, 0, 0, 0]);
  ...
  const completeMeal = async (meal) => {
    ...
    try {
      ...
      // ✅ Tab hi local state update karo
  ...
  useEffect(() => {
    if (!selectedPlanId || plans.length === 0 || selectedDayIndex === undefined) return;

    const loadProgress = async () => {
      try {
        console.log("Loading progress for Plan:", selectedPlanId, "Day:", selectedDayIndex); // ← Debug log add karo
        ...
        console.log("Loaded completed meals:", completedIds); // ← Debug log
