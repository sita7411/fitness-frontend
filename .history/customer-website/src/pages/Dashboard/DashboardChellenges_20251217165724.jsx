    ...
    const toggleGoalComplete = async (goalId) => {
        try {
            const res = await axios.post(
                `${API_BASE}/stats/toggle-goal`,  // ← Correct endpoint
                { goalId },
            ...
            if (res.data.allGoals) {
                ...
                // Confetti only when marking as COMPLETE (not when unchecking)
                const updatedGoal = res.data.allGoals.find(g => g.id === goalId || g._id?.toString() === goalId);
...
thumbnail: 
        (step.image && typeof step.image === 'string' && 
         ...
         !step.image.includes('undefined'))
            ? step.image 
            : '/trainer-3.jpg',  // ya apna better default image daal do                            description: step.description || step.notes || '',
