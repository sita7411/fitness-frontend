...
export default function DashboardChallenges() {
  ...
  return (
    <div className="min-h-screen bg-white p-6 md:p-10 rounded-lg">
            ...
            <>
              <div className="grid grid-cols-3 gap-3">
                ...
                <button
                  ...
                  className={`py-3 rounded-full font-medium transition ${
                    progressData.completedExercises.includes(exercises[currentIndex]?.id)
                      ? 'bg-gray-400 text-gray-600 cursor-not-allowed'
                      : 'text-white'
                  }`}
                  style={{ background: progressData.completedExercises.includes(exercises[currentIndex]?.id) ? '#9ca3af' : THEME }}
                >
      ...
      <AnimatePresence>
        {showMyGoals && (
          <motion.div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 p-4" onClick={() => setShowMyGoals(false)}>
            <motion.div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl border border-gray-100" onClick={e => e.stopPropagation()}>
              {/* Header & Body same as your original code */}
              {/* (keep your existing goals modal code – it already uses backend) */}
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
